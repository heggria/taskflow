...
- **Tree-reduce interruption accounting.** Intermediate-call usage now participates in live budget admission; budget truncation is explicit, preserves untouched inputs in the partial output, is never cached, and aborts fail instead of masquerading as a completed reduction.
- **Cwd and resume hardening.** Per-branch cwd overrides remain inside inherited canonical boundaries and cannot replace managed workspace/cwd bindings. Resume deep-clones retained state, preserves workspace provenance, accepts only failed/paused runs, and stamps the child with the current build identity.

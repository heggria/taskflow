...
- **Pi post-terminal hangs (#73).** Pi children now default to
  ...
  resource profiles. A validated final assistant answer plus
  `agent_end`/`agent_settled` enters a bounded terminal-candidate grace period;
...
- **Reproducible release tarballs.** Release packaging canonicalizes pnpm's
  publish-ready manifests, proves repeat-pack SHA512 stability, and reuses one
  immutable tarball set for consumer smoke, npm publish, provenance and

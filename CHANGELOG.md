...
- **0.2.0 Phase 2 / S0–S3 foundations (event-sourced kernel path):**
  ...
  - **S2 event kernel (strangler, default OFF, all 10 kinds):** `exec/step.ts` + `step-kinds.ts` + `exec/driver.ts` run **every** phase type (agent|script|map|parallel|reduce|gate|approval|loop|tournament|flow) when `RuntimeDeps.eventKernel` or `PI_TASKFLOW_EVENT_KERNEL=1`. Imperative path remains default. Nested `flow` via recursive kernel; gate block → run `blocked`; approval auto-reject without UI.

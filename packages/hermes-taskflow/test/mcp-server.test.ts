/**
 * MCP server binding tests for the hermes adapter.
 */
...
test("hermes mcp: initialize returns the protocol version + serverInfo", async () => {
	...
	assert.equal(res.result.serverInfo.version, "0.3.0-beta.2");
});
...
test("hermes mcp: taskflow_verify dispatches through the hermes binding (no execution)", async () => {
	const [res] = await rpcRoundtrip([
		{
			jsonrpc: "2.0",
			id: 3,
...
test("hermes mcp: makeToolHandlers exposes the tools", () => {

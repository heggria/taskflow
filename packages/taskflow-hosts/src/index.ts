 ...
 *
 * Each non-pi host (codex / claude / opencode) has two halves:
 ...
 *      claude-taskflow / opencode-taskflow) because it's the host-ecosystem
 *      install target (`codex plugin add`, `claude plugin install`, …).
 *
 ...
 * imports the runner from `taskflow-hosts`. The three legacy host packages
 * re-export from here (or import the runner directly into their `mcp/server`).
 *

...
function readGrokResult(pathname) {
...
function validate(manifest, manifestPath) {
	...
	if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 16) {
		fail("manifest.concurrency must be an integer between 1 and 16");
	}
	...
	const jobs = manifest.jobs.map((job, index) => {
		...
		if (seenCwds.has(cwd)) fail(`two jobs target the same cwd: ${cwd}`);
		seenCwds.add(cwd);
		...
		return { ...job, cwd, maxTurns, timeoutMinutes, retries };
	});
...
async function runJob(config, job, state, activeChildren, attempt = 1) {
	...
	return await new Promise((resolveJob) => {
		...
		child.on("exit", (code, signal) => {
			...
			if (code === 0) {
				try {
					grokResult = readGrokResult(stdoutPath);
				} catch (error) {
...
async function runPool(config) {
	...
	await Promise.all(
		Array.from({ length: Math.min(config.concurrency, config.jobs.length) }, () => worker()),
	);

...
| **Declarative DAG DSL** | **`pi-taskflow` — the only one. loop·tournament·cross-run cache shipped.** |

## 2. The Full Matrix

| Extension | Ver | Deps | Model | DSL | DAG | Fan-out | X-session resume | Gate | Approval | Save-cmd | Worktree | Loop | Tournament | Cache |
|---|---|:--:|---|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| **pi-taskflow** | 0.0.12 | **0** | per-phase override | **yes** | **yes** | **yes** `map` | **yes** (phase-hash) | **yes** | **yes** | **yes** `/tf:` | no | **yes** | **yes** | **yes** |
| pi-subagents | 0.28.0 | 3 | 8 builtin agents | no | no | static | UNVERIFIED | no | clarify | named wf | no | yes (review max-rounds) | no | no |
| pi-crew | 0.6.1 | 7 | role teams | partial | yes | yes | yes (async manifest) | yes | yes | UNVERIFIED | **yes** | UNVERIFIED | no | no |
| pi-pipeline | 0.4.22 | 2 | active model | no | fixed | no | yes (session-plan) | yes | UNVERIFIED | UNVERIFIED | no | no | no | no |
| pi-agent-flow | 2.3.5 | 2 | none | yes (flow JSON) | no | no | UNVERIFIED | no | no | UNVERIFIED | no | no | no | no |
| @0xkobold/pi-orchestration | 0.3.0 | 0 | typed registry + auto-select | no | no | yes (chain/parallel/fork) | no | yes (review_loop) | yes | no | **yes** (3 modes) | no | no | no |
| @fiale-plus/pi-orchestration | 0.1.2 | 0 | none (session cmds) | no | no | no | yes (session state) | no | no | no | no | yes (declarative `/loop`, no timer) | no | no |
| @fiale-plus/pi-rogue-orchestration | 0.1.17 | 0 | none | no | no | no | yes (loop state + history) | yes (goal-check) | no | no | no | **yes** (real setInterval loop) | no | no |
| @pi-agents/orchid | 0.1.0-β2 | 2 | agent prefs + override | **yes** (9-phase) | **yes** (wave sched) | **yes** | **yes** (batch state) | **yes** | **yes** (mailbox) | yes | **yes** (worktree.ts) | **yes** (Ralph loop) | no | no |
| @wkronmiller/pi-subagent-extension | 0.1.0 | 0 | inherits parent | no | no | no | yes (run records + events) | no | yes (mailbox) | no | no | no | no | no |
| @mediadatafusion/pi-workflow-suite | 0.0.11 | 3 | per-role model/thinking | no | no | UNVERIFIED | yes (mode persist) | **yes** (PASS/PARTIAL/FAIL) | **yes** | yes | no | yes (repair/retry) | no | no |
| @tianhai/pi-workflow-kit | 0.17.1 | 0 | active model | no | no | no | no | yes (checkpoint gates) | yes (pause) | yes (slash cmds) | no | no | no | no |
| @zhushanwen/pi-workflow | 0.2.2 | 0 | `agent({model})` | **yes** (JS API) | no (linear chain) | **yes** (`parallel()`) | **yes** (8-state + JSONL + call cache) | no | no | yes (call cache) | no | no | no | no |
| @rajeshkrishnamurthy/pi-workflow-team | 0.1.3 | 0 | UNVERIFIED | UNVERIFIED | UNVERIFIED | UNVERIFIED | UNVERIFIED | UNVERIFIED | UNVERIFIED | UNVERIFIED | UNVERIFIED | UNVERIFIED | UNVERIFIED | UNVERIFIED |
| @dreki-gg/pi-subagent | 0.9.1 | 0 | 10 agents, per-run override | no | no | no | partial (fork-at clone) | no | per-agent | no | no | partial (prove-before-fix) | no | no |
| @melihmucuk/pi-crew | 1.0.20 | 0 | 6 agents, override | no | no | yes (manual spawn) | **yes** (survives /new /resume /fork) | no | no | partial | no | partial (multi-turn) | no | no |
| @gotgenes/pi-subagents | 14.0.1 | 1 | fuzzy haiku/sonnet | no | no | no | **yes** (resume by agent ID) | no | per-agent | no | **yes** (auto-commit branches) | no | no | no |
| @narumitw/pi-subagents | 0.1.37 | 1 | inherits | no | no | no | no | no | per-agent | no | no | no | no | no |
| @mjakl/pi-subagent | 2.1.0 | 0 | per-agent frontmatter | no | no | no | no | no | per-agent | no | no | no | no | no |
| ultimate-pi | 0.26.0 | **16** | dedicated harness agents | yes (YAML contracts) | **yes** (plan-time) | no | **yes** (active-run + trace) | **yes** (3-tier) | **yes** | **yes** (`/harness-auto`) | no | **yes** (steer loop) | no | no |
| gentle-pi | 0.4.5 | 1 | per-agent routing UI | yes (OpenSpec SDD) | no (sequential) | no | **yes** (artifact model) | **yes** (blind dual review) | **yes** | partial | no | **yes** (SDD apply→verify→sync) | no | no |
...
2. **The only declarative `loop` and `tournament` phases.** No other Pi extension
   combines iterative convergence (`loop`: re-run until a condition, convergence fixpoint,
   or cap) with best-of-N judging (`tournament`: spawn N variants, a judge picks the
   winner or aggregates) inside a DAG — both shipped and battle-tested.
3. **The only cross-run memoization with deterministic freshness guards.**
   Content-addressed cache with `git:HEAD`, `glob:`, `file:`, `env:` fingerprints and
   configurable TTL — reuse unchanged phases across independent runs for $0.00.
4. **The only package combining DAG + gate + approval + budget + save-as-cmd + zero deps.**
   `@mediadatafusion` (3 deps) and `ultimate-pi` (16 deps) get gate+approval+save but
   have no user-definable DAG DSL.
5. **The only phase-level input-hash cross-session resume.** Everyone else resumes by
   ...
   downstream re-run.
6. **Context isolation as a design pillar.** Only the final phase reaches your
   conversation; intermediate transcripts are withheld from the LLM. Others return
   results inline or to the parent session.
7. **Lowest ceremony-to-power ratio among feature-rich packages.** 0 deps, one install,
...
*Loop-until-done was previously listed here; it shipped in v0.0.12. Tournament and
cross-run memoization also shipped — the gaps are now worktree, non-blocking async, and
...
> DAG-shaped subagent pipeline with loop, tournament, and cross-run memoization —
> you save it as a one-word command, with zero runtime dependencies and context
> isolation by design.**

No other extension can truthfully claim *all six*: (1) declarative JSON DSL,
(2) runtime-owned DAG topology, (3) cross-session resume with phase-hash caching,
(4) save-as-`/tf:<name>`, (5) zero runtime deps, (6) loop + tournament + cross-run
cache in one package. The closest each miss ≥2: `orchid` (fixed pipeline, 2 deps,
no user DSL); `ultimate-pi` (16 deps, heavy ceremony); `pi-crew` (7 deps,
team-init bootstrap).

## 6. Threats to watch

*Updated 2026-06-08: loop, tournament, and cross-run memoization have shipped,
so prior high-priority threats (rogue for loop, orchid for tournament) are
substantially mitigated. The remaining gap is non-blocking async.*
...
1. ~~Loop-until-done shape~~ → **RESOLVED** (shipped: DAG-native `loop` phase with `until` condition + `convergence` + `maxIterations`).
2. **Worktree isolation** — required or out of scope? Four competitors ship it. Candidate: a `worktree` phase field.
3. **Non-blocking execution** — v2 priority vs. static-verification? Two competitors prove users want it.
...
6. **Evaluation framework** — should we add builtin eval scorers (`@mastra/evals`-style) or keep `gate` as the only quality primitive? The gap matters as Mastra raises the bar.

...
Wire-freeze gate for taskflow 0.3.0 core plus the provisional P17 v5 beta.2 browser target. Master RFC v7.7 is frozen; detail lives here + TypeBox.
...
P1–P16 gate the core 0.3 wire. P17 v5 is the independent provisional beta.2 browser target and is not wire-frozen until its TypeBox, fixed-position deterministic Task/observation/decision/error projection, complete content catalogs, verification, response budgets/pagination, handler, authoritative-source, codec, compatibility, HTTP/static security, comprehension/linguistic-QA, and packaged-E2E gates pass. Files-only storage is specified in P14. P16 is not foldable into P13.

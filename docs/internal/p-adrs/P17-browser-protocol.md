...
This document is the **v5 target**, while the checked-in `web-protocol.ts` is a pre-v5 implementation and both presentation modules/content catalogs are absent. The module becomes canonical executable wire authority only after its declared endpoint registry, strict producer/tolerant presentation-consumer TypeBox codecs, deterministic presentation family, complete content catalogs, golden fixtures, static-delivery contract, and handlers conform to every requirement below. Until then, neither document nor code may claim wire freeze.
...
Every string/preview and page element has schema bounds; large payloads become authorized `ArtifactRef`s. Every array embedded in a non-page DTO has an explicit schema maximum no greater than 200 unless this P-ADR names a lower bound; if it is a summary rather than a pageable first page, it also carries the total and an explicit summary/truncation flag and never claims completeness. Semantically complete collections that can exceed the bound require a listed continuation endpoint. If one supposedly bounded element still exceeds 64 KiB or a non-page response exceeds its budget, the handler sends no partial JSON and returns sanitized 500/`TF_DURABILITY_FAILED`; this is a conformance failure, not permission to truncate silently.
...
- `{projectId, controlDomainId, runId}`, stable `workspaceDisplayName`/`displayTitle`, current `runVersion`, created/updated timestamps;
- separate canonical RunStatus and RunStage, needs-operator/recovery state, side-effect uncertainty;
- Program/source identity, FlowIR hash, BoundPlan hash and a bounded reloadable BoundPlan provenance/summary; large source documents are authorized artifact references;
- `fragmentCount` plus the first `WebPage<WebBoundFragmentSummary>` using default 50/maximum 200; every summary includes hash, parent/link/causation provenance, dynamic origin counts, and its continuation identity;
- graph inventory totals and a bounded first `WebRunGraphView` chunk, including dynamic/static origin and dependency/control semantics;
- node and Attempt/provider totals plus only the bounded graph/summary pages defined here;
- a bounded first `WebPage<WebTimelineEvent>` plus continuation cursor;
- resolved safe input/output references; SecretRefs remain opaque/redacted;
- cache/reuse summary, why-stale availability, replay availability;
- `artifactCount` plus the first `WebPage<WebArtifactRef>` using default 50/maximum 200; each row binds role, digest/size/MIME, reachability/redaction, Receipt relation, and current verification-check summary without embedding artifact bytes;
- an optional bounded Receipt summary containing immutable identity/assurance totals and the first event-manifest page using default 100/maximum 200; complete evidence is recovered through the Receipt and artifact continuation endpoints, never one unbounded JSON object;
- policy/capability decisions relevant to this Run;
- `sourceObservation`, canonical `presentation`, and server-computed `availableActions`.

Empty fragments, attempts, artifacts, or Receipt are valid only when domain truth says they are empty/absent. An empty graph chunk is valid only when the bound query has no matched nodes and has no continuation. WebGateway may not substitute empty collections because a read service is missing.
...
`WebNodeDetail` requires node identity, definition/instance provenance, dependency identities, status, `attemptCount`, the first `WebPage<WebAttemptSummary>` using default 50/maximum 200, current provider observation, timing/usage, safe input/output/artifact references, cache/fingerprint explanation, linked fragments/children, relevant timeline refs, `sourceObservation`, and actions when any are valid. Complete Attempt history is recovered only through the bound attempts continuation endpoint.
...
`WebAttentionItem` requires stable kind/id, project/domain/Run or reservation identity where applicable, severity, first/last observed timestamps, safe explanation, side-effect uncertainty, recovery action, `disposition:"needs-user-input"|"status-only"|"diagnostic"`, source observation, authoritative-detail link, and current `availableActions`. It is a derived queue, not a mutation authority.
...
`WebReceiptView` contains the complete bounded immutable Receipt core (identity, assurance/guarantee, provider/result provenance, usage/cost and manifest roots/counts), full `WebVerificationPresentation`, `artifactCount`, and the bounded event-manifest page selected by `WebReceiptQuery` using default 100/maximum 200; an empty cursor selects the first page embedded by Run detail. Artifact rows continue through the Run artifact endpoint. Following stable cursors under one Run/Receipt version recovers the complete semantic evidence exactly once. A summary card alone is not a conforming Receipt surface, one response is never required to contain an unbounded manifest, and Receipt presence alone never produces `verified`.
...
- Clock rollback cannot extend an issued cursor beyond its encoded `expiresAt`.
...
`WebPageCursorPayload` requires version/kind, listener id, issued/expiry, principal hash, normalized query hash, stable sort, exclusive keyset `after` equal to the last element returned by the preceding page, the exact `registryContext` discriminated union from §2.3, ordered visible project identities, every project’s `{nextCommitSeq,minAvailableCommitSeq}`, and any resource version binding required by a detail collection such as `{runId,runVersion}`. The next page starts strictly after that tuple under the encoded sort direction.

`WebStreamCursorPayload` requires the same listener/principal/registry-context/mount/watermark binding but no query, sort, or page keyset.

Continuation verifies MAC, payload schema, kind, protocol/listener, expiry, principal, query/sort where applicable, registry context, visible mounts, and compaction floor. Auto requires a real current registry revision. Standalone requires the literal `"standalone"` sentinel and exactly one matching mount. Any failure that represents an old but once-valid checkpoint returns `TF_CURSOR_EXPIRED` with `recoveryAction:"refresh"`. Malformed/tampered/cross-kind/mode-invalid cursors return `TF_INVALID_ARGUMENT` without revealing which check failed. A stream/page cursor is never accepted in the other endpoint.
...
| Header count | `server.maxHeadersCount = 64` |
| Header completion | `server.headersTimeout = 10_000` ms |
| Non-stream request/body completion | `server.requestTimeout = 30_000` ms |
...
Polling fallback uses the same authorized GET DTOs and page/checkpoint rules at a server-advertised minimum interval of 3 seconds. It does not introduce a second event model.
...
The existing `web-protocol.ts` is explicitly expected to change; `web-presentation-server.ts`, `web-presentation-client.ts`, the content catalogs, and the joint reference-set manifest/fixtures do not yet exist. At minimum, conformance work must add or verify:

- Overview, project detail, bounded fragment/graph/timeline/Attempt/artifact/Receipt continuation, node detail, approval detail, why-stale, replay, recompute-preview, logout, and revoke-all schemas/endpoints;
- the 29-row sole `WEB_ENDPOINTS` registry plus generated client/router wiring and `scripts/check-web-protocol-docs.mjs` drift guard;
- stable Workspace/Task display-name fields with deterministic provenance;
- durable/reconstructable node presentation metadata plus canonical Task/observation/decision/error projections, complete `taskflow-content.v1` catalogs, and six-state verification schemas/functions;
- bootstrap/asset-manifest content version, locale, keyset and catalog digest fields plus fail-closed mixed-build validation;
- closed feature and command enums plus per-resource `availableActions`;
- complete bounded Run detail groups backed by durable BoundPlan/Fragment/node/Attempt/artifact/Receipt sources and continuation services;
- exact status/error mapping, full-envelope response budgets, media/size limits, low-level HTTP limits/timeouts, cookie/CSRF/session constants, and SPA `Accept` negotiation;
- exact cursor byte encoding/MAC input/key/TTL rules and known-answer vectors;
- exact SSE framing/closed enums/Last-Event-ID/limits/backpressure behavior;
- artifact redaction/MIME/inline/download/limit/pre-verification snapshot rules;
- application shell/static route, asset manifest, cache, MIME, CSP/header and source-map rules;
- strict-producer/tolerant-presentation-consumer schemas plus endpoint/projection/content/catalog/compatibility golden fixtures.

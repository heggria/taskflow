...
- optional bounded `receipt:WebReceiptDetail` containing immutable identity, BoundPlan/BoundFragment relation, commit range, assurance, build info, and at most the first 200 event/artifact references. The preview is never a completeness signal. Exact counts/digests, complete verification, and paged event-manifest evidence come from `WebReceiptView`; complete authorized artifact rows continue through the Run artifact endpoint;
...
`WebReceiptView` contains the bounded immutable `WebReceiptDetail` core (identity, BoundPlan/BoundFragment relation, commit range, assurance, build info, and bounded event/artifact previews), full `WebVerificationPresentation`, additive top-level `eventManifestCount`, `eventManifestDigest`, `artifactCount`, and `artifactRefsDigest`, plus the bounded event-manifest page selected by `WebReceiptQuery` using default 100/maximum 200; an empty cursor selects its first page. `eventManifestDigest` is SHA-256 over the canonical ordered Receipt event-id array, while `artifactRefsDigest` is SHA-256 over the canonical immutable Receipt artifact-reference array. The producer requires the three new count/digest fields; the additive consumer accepts the committed pre-count response and treats its at-most-200 Receipt arrays as complete only for that older response shape. Artifact rows continue through the Run artifact endpoint. Following stable Receipt cursors under one Run/Receipt version recovers every event-manifest entry exactly once; export must also verify count, order, commit range, preview prefix, and digest before producing a file. A summary card alone is not a conforming Receipt surface, one response is never required to contain an unbounded manifest, and Receipt presence alone never produces `verified`.
...
No `payload:unknown` branch exists. The browser command namespace is listener-global, not project-local. Before any mutation, the daemon atomically claims the `commandId` in the coordinator with the exact authority project/domain, current principal, and canonical Web request hash. The same tuple is idempotent; a different authority or hash is `TF_IDEMPOTENCY_CONFLICT`, and a different principal is `TF_CROSS_PRINCIPAL_COMMAND`. A simultaneous cross-project reuse has exactly one winner. `GET /commands/:commandId` resolves the durable claim to one authority and performs live re-authorization before disclosure. If that claimed home authority is not currently mounted/readable, dispatch and query fail closed as durability/recovery failures; they never masquerade as “command not found” and never reroute to another project. A bounded legacy project scan is migration-only when no coordinator claim exists; multiple legacy matches fail closed as corrupt authority.
...
Read-service caches are rebuildable optimizations keyed by the complete source observation. Auto mode compares current registry/mount/project watermarks; standalone mode still probes the exact current project/domain watermark on every cache decision. Absence of a registry resolver never turns a time-to-live into authority and cannot keep a changed standalone ControlStore snapshot valid.

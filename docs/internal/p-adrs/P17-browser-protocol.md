...
> Status: **Provisional** (0.3.0-beta.2 initial browser schema baseline; wire freeze not yet declared)
...
P1–P16 define control-plane authority and storage semantics. They do **not** define an HTTP/browser protocol. P17 is the initial authority for browser DTO names, HTTP envelopes, pagination, aggregate cursors, session exchange, command dispatch/query, SSE frames, and artifact delivery. It remains provisional until the codecs, handlers, compatibility fixtures, and hostile-input tests listed below exist.
...
- Every path-capable identifier uses the core SafeId contract: 1–128 characters, first character alphanumeric, remaining characters `[A-Za-z0-9._-]`, with `..`, slash, backslash, and NUL rejected at schema, host, and store boundaries.
...
| `GET /api/v1/coordinator` | none | `WebCoordinatorSummary` | Live session; coordinator lock snapshot |
| `GET /api/v1/coordinator/reservations/:reservationId` | `WebReservationParams` | `WebReservationDetail` | Live session; current reservation revision and binding |
...
- Status ordering uses the canonical rank `running=0, paused=1, blocked=2, unknown=3, failed=4, cancelled=5, completed=6`; no locale or enum-declaration order is consulted.
...
## Page and stream cursor algorithms

The browser sees opaque, authenticated encodings and may not parse or construct them. Pagination uses `WebPageCursorPayload`; SSE resume uses the distinct `WebStreamCursorPayload`.
...
3. A page cursor hashes the normalized query and principal authorization context, records the stable sort plus the exclusive keyset tuple `after={sortValue,projectId,controlDomainId,runId}`, `issuedAt`, and bounded `expiresAt`, then signs/MACs the payload with an ephemeral daemon secret.
...
A stream cursor contains the registry revision and project commit watermarks but no page keyset. Both vectors are checkpoints, not a global causal clock. SSE delivery order and cross-project timestamp order never become global commit order.
...
Presence in the union defines the versioned shape, not runtime availability. `edit-approval` must return `TF_FEATURE_REQUIRED` and remain unoffered until edited BoundPlan persistence plus dispatcher handoff exist; it may never manufacture provider completion or a Receipt from the edit decision itself.

`WebCommandOutcome` is also a closed union. `pending` cannot carry an error, `completed` cannot carry an error, `failed` and `rejected` must carry a `ControlError`, and `not-found` cannot pretend to have a request hash or commit range.
...
`force-release` requires reservation id, observed state, reservation revision, coordinator fencing epoch, bound project id, bound control-domain id, bound Run id, and the exact P16 acknowledgement string. The coordinator derives the request hash from this typed request and compares every field under its mutation lock before release.

## Hard-GA Run detail

`WebRunDetail` carries the graph nodes and edges, BoundPlan and BoundFragment provenance, attempt summaries, a cursor-bounded timeline page, artifact references, why-stale and replay summaries, and the complete Receipt assurance/event manifest when a Receipt exists. A summary-only receipt or node list without edges is not a conforming Hard-GA response.
...
- Golden JSON compatibility fixtures for every DTO branch plus real HTTP/SSE codec round-trips.
- Handler tests proving coordinator/run-detail reads are sourced from current authorities rather than browser caches.

## Freeze condition

This revision is the beta.2 **initial schema baseline**, not a wire freeze. P17 may become `Accepted / wire-frozen` only after every endpoint has an implemented handler, request and response codec round-trips against this module, all verification gates above pass, and compatibility fixtures prove additive/minor versus breaking/major behavior. Until then, schema changes require a P17 revision but do not claim wire compatibility.

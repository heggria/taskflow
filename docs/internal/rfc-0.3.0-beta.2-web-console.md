...
`7e555c7d7f39109ee89a502f0d818cc34f1ce7fa` and source digest
...
1,137.5 ms cold-Home p95, 918.5 ms warm-Home p95, 50.3 ms cached-Pro p95,
178.1 ms event-to-painted-detail p95 (78.3 ms commit→receipt and 105.3 ms
receipt→paint diagnostic p95s), 6.7 ms list response, and 0.0805 maximum CLS
session window. Owner-ready and launch-to-useful p95s are retained separately
as 8.779 s and 9.772 s. The Simple shell is 209,410 gzip bytes. The schema-v4
...
9.75/8.42/7.44 and 7.84/8.19/7.63. Because this is not the canonical M2 Mac
mini profile and the run recorded high pre-load, every latency result remains
informational even where a number is below the canonical budget. The unloaded
canonical gate stays open.
...
- [ ] Cross-browser matrix on supported current Chrome/Edge, Firefox, and
  ...
  session-ended state instead of a generic failure. That smoke does not claim
  an unlocked Safari private-window actor. Native Edge, VoiceOver and the
  remaining manual matrix remain outstanding. The automated Firefox 151.0 and
  WebKit 26.5 lanes remain supporting evidence, not substitutes for those
  native-product lanes.
...
Implementation: The React WebUI, loopback WebGateway, session/CSRF/static delivery, SSE plus bounded polling fallback, generated client, Simple/Pro projections, durable command recovery, Pro graph/timeline/evidence/replay/technical panels, and session revocation controls are executable. Packaged CLI → daemon → multi-project real-browser E2E covers independent nonce-host listeners, live cancel, restart-safe contextual approval and downstream Receipt, private-checkpoint non-reachability, deterministic Receipt export, zero-write/zero-provider replay, SSE/polling projection equivalence, every current GET polling surface, keyboard/focus semantics, 320px layout, CSP/runtime-style guards and Chromium axe. Chromium 149.0.7827.55, Firefox 151.0 and WebKit 26.5 pass the packaged matrix with zero application console/page errors; native Chrome 150.0.7871.184 passes the same complete path. Native Safari 26.3 on macOS 26.3 has separately scoped read/keyboard plus allow, reject, cancel, current-session, listener-wide revocation and peer-invalidation smokes. Cancel proves the formerly active node settles as stopped; peer invalidation proves a Safari target renders the localized session-ended state after a separate authenticated P17 session revokes all, but does not claim a second unlocked Safari cookie jar. These AX/native-input smokes do not replace VoiceOver or other assistive-technology review. All ten paged surfaces, recursive schema branches, exact 100-MiB artifact transport, the five-minute absolute deadline, real SSE backpressure, cross-runtime cursor invalidation, canonical static-path/hostile-package and singleton/control-header corpora pass. Immutable source `7e555c7d7f39109ee89a502f0d818cc34f1ce7fa` produces a 209,410-byte gzip Simple shell and a schema-v4 M3 Pro/Node 24 clean-source 30-sample structural pass over 100 projects, 10,000 Runs and 2,000 graph nodes. It remains non-canonical performance evidence; the M2/Node 24 gate is still required. Distinct immutable candidates `d8df9c650b68e4c0854aaad03e6da3ce0dc3352f` and `7e555c7d7f39109ee89a502f0d818cc34f1ce7fa` pass old-client/new-server and new-client/old-server with Web v1 bootstrap and zero application console/page errors. Human product/content approval, five fresh English participant records, two native zh-CN reviews, native Edge/assistive-technology evidence, canonical performance evidence, reviewed-tip compatibility/candidate evidence, edited approval and separately gated recovery dispatch where required for GA, and a separate wire-freeze review remain mandatory.
```

...
`d8df9c650b68e4c0854aaad03e6da3ce0dc3352f` and source digest
...
1,268.1 ms cold-Home p95, 904.7 ms warm-Home p95, 30.7 ms cached-Pro p95,
151.1 ms event-to-painted-detail p95 (76.9 ms commit→receipt and 95.0 ms
receipt→paint diagnostic p95s), 8.6 ms list response, and 0.0784 maximum CLS
session window. Owner-ready and launch-to-useful p95s are retained separately
as 25.700 s and 27.175 s. The Simple shell is 209,367 gzip bytes. The schema-v4
...
8.05/8.69/8.62 and 7.56/8.73/8.86. Because this is not the canonical M2 Mac
mini profile and the run recorded high pre-load plus large process-owner
launch outliers, every latency result remains informational even where a
number is below the canonical budget. The unloaded canonical gate stays open.
...
- [ ] Cross-browser matrix on supported current Chrome/Edge, Firefox, and
  ...
  verifies listener close. Native Edge, Safari cancel/peer-session cases,
  VoiceOver and the remaining manual matrix remain outstanding. The automated
  Firefox 151.0 and WebKit 26.5 lanes remain supporting evidence, not
  substitutes for those native-product lanes.

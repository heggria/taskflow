...
| Timeline | `GET .../runs/:runId/timeline`; `WebPage<WebTimelineEvent>` with projected whole-message summary | Simple activity summary; technical timeline Pro, localized with the selected application locale | **Must ship** |
| Evidence and artifact | `GET .../runs/:runId/receipt`, `GET .../runs/:runId/artifacts`, plus `GET .../artifacts/:digest`; paged `WebReceiptView`, `WebArtifactRef` with projected disclosure state | Simple verification/result; complete pageable evidence Pro, explicit sensitive acknowledgement, no secret download control | **Must ship** |
...
1. Generate a listener-global `commandId` before submit and retain it until a committed outcome is read. Before mutation the daemon atomically binds it to one authority, principal, and canonical Web request hash in the coordinator; a simultaneous cross-project reuse has exactly one winner.
...
- Command recovery fixtures cover durable listener-global route claims, exact idempotent retry, cross-project and cross-principal conflicts, simultaneous collision, unambiguous live-reauthorized lookup, and fail-closed legacy ambiguity.
...
- Artifact tests prove the entire digest/length is verified against an immutable snapshot before response headers or any body byte; sensitive disclosure requires a deterministic acknowledgement dialog and request header, secret disclosure has no download action, success reports the redaction class, and a listener-wide pessimistic 100-MiB permit bounds concurrently materialized artifact bytes without a second full-body copy.
- Render evidence v2 binds screenshots to an exact tracked-clean source commit, Web build id, and asset-manifest digest. Evidence checks reject relevant source drift, and every human/linguistic/browser/assistive-technology record names the exact render candidate rather than an arbitrary ancestor.
...
- [ ] Cross-browser matrix on supported current Chrome/Edge, Firefox, and
  ...
  decline/acknowledgement contract. Native Safari 26.3 now has four
  current-source scoped records bound to `c0454001`: read/keyboard/graph/
  evidence, Allow plus revoke-all, Reject plus current-session logout, and
  live cancel. Current-source independent peer/two-cookie invalidation, native
  Edge, VoiceOver and the remaining manual matrix stay outstanding. Automated
  WebKit and AX-driven Safari observations remain supporting evidence, not a
  substitute for a native Safari product/assistive-technology lane.
...
Implementation: The React WebUI, loopback WebGateway, session/CSRF/static delivery, SSE plus bounded polling fallback, generated client, Simple/Pro projections, durable command recovery, Pro graph/timeline/evidence/replay/technical panels, and session revocation controls are executable. Packaged CLI → daemon → multi-project real-browser E2E covers independent nonce-host listeners, live cancel, restart-safe contextual approval and downstream Receipt, private-checkpoint non-reachability, deterministic Receipt export, zero-write/zero-provider replay, SSE/polling projection equivalence, every current GET polling surface, keyboard/focus semantics, 320px layout, CSP/runtime-style guards and Chromium axe. Immutable packaged/evidence build `130e1a3f7c0bf991c96d6e0192a2517be43ff827`, bound by evidence commit `3823c32d1e8992d86d568e54a69160b15c48f593`, passes bundled Chromium 149.0.7827.55, Firefox 151.0, WebKit 26.5 and native Chrome 150.0.7871.184 on the first attempt of every lane with zero application console/page errors. Every lane proves sensitive-artifact disclosure, decline-without-request, exact acknowledgement on confirm, SSE/polling equivalence, approval/cancel, Receipt/export/replay, session invalidation and 320px behavior. All ten paged surfaces, recursive schema branches, exact 100-MiB artifact transport, the five-minute absolute deadline, real SSE backpressure, cross-runtime cursor invalidation, canonical static-path/hostile-package and singleton/control-header corpora pass. The pinned Node 22.19/24.18/26.5 matrix passes 63/63 tests per runtime. Historical build `aa34369a` and current build `130e1a3f` pass real old-client/new-server and new-client/old-server packaged compatibility with Web v1 bootstrap and zero application console/page errors. The exact seven §19.4 usability tasks, timing/moderator/privacy rules, eight severity-1 classes, 356-key `zh-CN` coverage, observed-behavior native/AT template, and immutable completed-evidence verifier are executable and self-tested without fabricating human results. Current build `130e1a3f` produces a 211,426-byte gzip Simple shell and a schema-v4 M3 Pro/Node 24 clean-source 30-sample structural pass over 100 projects, 10,000 Runs and 2,000 graph nodes; its non-canonical 291.8-ms event-to-visible p95 is above the canonical 250-ms budget and is not presented as a pass. The M2/Node 24 gate is still required. Native Safari 26.3 has four current-source scoped records bound to `c0454001` for read/keyboard/graph/evidence, approval Allow/Reject, cancel, current-session logout and listener-wide revoke-all. They do not claim VoiceOver, an independent peer/two-cookie path, console/CSP/axe automation or human approval.

Implementation source commit `6f18a4eff57f1774328808cdfea22e58178555e4` closes dynamic descendant execution, provider-owned cancellation, unmounted command authority, standalone authority cache invalidation, bounded complete Receipt export, single-copy artifact delivery, launch-fragment erasure and React StrictMode lifecycle races. Its catalogs contain 168 projected + 188 static = 356 bilingual keys. Render-evidence v2 retains 149 screenshots under `artifacts/web-reference/6f18a4eff57f/`, bound to the exact Web build and asset-manifest hashes; automated render and usability-material checks pass, while the record remains explicitly unapproved. `docs/internal/webui/immutable-candidate-evidence-v2.md` binds the current compatibility, Node, browser, native Safari scoped and non-canonical performance reports. Human product/content approval, five fresh English participant records, two native zh-CN reviews, current-source Safari independent peer/two-cookie invalidation, native Edge/assistive-technology evidence, canonical performance evidence, reviewed-tip evidence, edited approval and separately gated recovery dispatch where required for GA, and a separate wire-freeze review remain mandatory.
```

...
- [ ] Cross-browser matrix on supported current Chrome/Edge, Firefox, and
  ...
  decline/acknowledgement contract. Native Safari 26.3 now has five
  current-source scoped records bound to `c0454001`/`f6dfc801`: read/keyboard/
  graph/evidence, Allow plus revoke-all, Reject plus current-session logout,
  live cancel, and independent normal/private two-cookie-jar listener-wide
  revocation. Native Edge, VoiceOver and the remaining manual matrix stay
  outstanding. Automated WebKit and AX-driven Safari observations remain
  supporting evidence, not a substitute for a native Safari product/
  assistive-technology lane.

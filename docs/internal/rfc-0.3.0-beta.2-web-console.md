...
| **W11** | Stack: React 19.2 + Vite 8.1 static SPA + TypeScript 7 + TanStack Router/Query + React Aria Components + `@xyflow/react`; Node ≥22.19 compatibility, Node 24 LTS primary, Node 26 forward CI; no Next.js server or SSR. |
...
| Approve/reject decisions | **Capability-gated** | P15 CAS plus durable original BoundPlan reload and dispatcher handoff; decision never implies provider completion |
...
The first beta.2 may therefore ship a complete, approachable observe/evidence workspace plus a truthful cancel path, with replay and full diagnostics in Pro while approval decisions and high-risk recovery writes remain wholly absent. That is a complete beta.2 according to W15, not a partial implementation disguised by disabled controls.
...
| `WebAttentionItem` | derived actionable condition with recoveryAction and sideEffects. |
...
- The opaque page cursor is a signed/MACed vector checkpoint bound to protocol version, principal hash, normalized query hash, stable sort, the exclusive keyset tuple of the last row actually returned, exact auto/standalone registry-context discriminant, and every visible project’s `(nextCommitSeq, minAvailableCommitSeq)`. The next page starts strictly after that tuple in the bound sort direction; a cursor never points at the first omitted row. Standalone uses the literal P17 sentinel and exactly one mount; it does not fabricate a registry revision. The SSE stream cursor is a distinct registry-context/project-watermark checkpoint and never carries page keyset state.
...
| Approvals | `GET /approvals`, `GET .../approvals/:approvalRequestId`; `WebApprovalSummary`, `WebApprovalDetail` | Simple Needs your input; CAS/evidence Pro | **Must ship read-only**; decisions capability-gated |
...
- Until that dispatcher can reload the exact durable BoundPlan/BoundFragment, the implementation returns `TF_RECONCILE_REQUIRED`, keeps the command `accepted`, and marks the Run `unknown/reconciling + needsOperator`; beta.2 cannot pass the approval GA gate in this state.
...
- generate `packages/taskflow-web/test/fixtures/perf-v1` from fixed seed `0x54465331`: 100 projects, 10,000 Run summaries, 200 active Runs, 50 approvals, 100 attention items, one 2,000-visible-node graph, and deterministic artifact/Receipt distributions;
...
- run three discarded warmups followed by 30 measured samples per scenario, in a fixed scenario order rotated between runs;
- compute p50/p95 with nearest-rank (`ceil(p × n)`) over raw samples; store every sample, not only aggregates;
- measure interaction latency with browser `performance.mark`, event visibility from committed server timestamp plus monotonic client receipt/render marks, bundle size from packaged gzip bytes, and CLS through `PerformanceObserver`;
...
- Auto and standalone cursor payloads use their exact discriminated registry context; no fabricated standalone registry revision.
...
The current control-host scaffold intentionally fails closed for approve/edit dispatcher gaps: it creates no synthetic terminal state or Receipt, and `edit-approval` is not an offered capability. That behavior removes false success but does **not** satisfy the approval checkboxes below. GA still requires durable original/edited BoundPlan persistence, dispatcher handoff, and real provider-outcome tests.
...
- [ ] Visual/interaction regressions remain gated by the complete Simple/Pro light/dark state matrix.
...
Executable schema: Existing web-protocol.ts is pre-v5; server/client presentation modules, content catalogs, and reference-set manifests/fixtures are absent; endpoint/DTO/pagination/projection/content/compatibility/cursor/session/SSE/artifact/static-delivery conformance delta remains
Read model: §7.5 is normative; required ControlHost reads and durable plan/fragment/node/attempt sources are not yet complete
Implementation: WebGateway/UI not started; next gate is the joint §15.2 screen + §10A copy matrix + P17 v5 fixture/provenance + narrow/comprehension approval, then schema/projection/content/read-service/static-delivery conformance
```

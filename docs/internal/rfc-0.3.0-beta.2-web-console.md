...
> P1–P16 remain authoritative for core control-plane semantics. **P17 provisionally owns the HTTP/browser schema baseline** and [`web-protocol.ts`](../../packages/taskflow-control/src/web-protocol.ts) is its canonical TypeBox surface. This RFC defines the product and implementation boundary; it does not create a second execution or authority model. P17 is not wire-frozen until its handlers, codecs, compatibility fixtures, and hostile-input tests pass.
...
Browser DTOs are derived views, not new domain entities. The names below are the provisional P17/TypeBox baseline; there is no second illustrative envelope:
...
| `RunDetail` | Run projection + graph edges + BoundPlan/Fragments provenance + node/attempt projections + timeline page + artifact refs + full Receipt manifest/assurance + why-stale/replay. |
...
- The opaque page cursor is a signed/MACed vector checkpoint bound to protocol version, principal hash, normalized query hash, stable sort, exclusive keyset tuple, registry revision, and every visible project’s `(nextCommitSeq, minAvailableCommitSeq)`. The SSE stream cursor is a distinct registry/project-watermark checkpoint and never carries page keyset state.
...
```text
0. Validate Web consumer needs; establish the provisional P17 schema independently from P1–P16
1. Browser-safe TypeBox protocol + projection/command/page-cursor/stream-cursor/SSE/artifact contract tests
...
The WebUI starts after the control wire and thin CLI surfaces are coherent. Browser-consumer requirements belong in P17 rather than being retroactively attributed to P1–P16. Wire freeze is a later gate after real handler/codec and compatibility evidence, not an input assumption to step 1.
...
Browser wire: P17 provisional beta.2 schema baseline; web.v1 TypeBox exists, handler/codec/compatibility gates remain

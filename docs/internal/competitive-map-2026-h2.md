...
- [ ] **0.3.0-beta.2 WebUI** follows [`rfc-0.3.0-beta.2-web-console.md`](./rfc-0.3.0-beta.2-web-console.md) v7: Simple task workspace by default, complete Pro disclosure over identical authority, deterministic fixed-position Task/observation/decision/error/verification presentation, bounded evidence pagination, calm-truthful content catalogs, joint reference screen + copy matrix + golden fixture + narrow/comprehension gate, hard-GA observation/approval/cancel/evidence core, read-only policy explanation, and independently capability-gated risky writes

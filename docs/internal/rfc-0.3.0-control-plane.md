...
> **Branch:** `feat/0.3.0`
> **Date:** 2026-07-22
> **Approver action:** Architecture **Approved**; protocol model **Approved with conditions**; Steps 1–2.5 **go**; core wire freeze **not** yet (P1–P16 validation); beta.2 P17 is a provisional browser schema baseline, not a wire freeze.
...
> | Browser wire baseline | **P17 provisional for beta.2 implementation; not wire-frozen** |
...
9.   Thin MCP + CLI
10. WebUI 0.3.0-beta.2 (provisional P17 browser schema + Web Console RFC implementation order)
```
...
P17 is not retroactively folded into the core gate above: it is the independent provisional beta.2 browser schema covering HTTP envelopes, DTOs, pagination, page/stream cursors, session exchange, command dispatch/query, SSE, and artifacts. Its own handler/codec/compatibility evidence is required before wire freeze.
...
Browser wire baseline: P17 provisional for beta.2 implementation; handler/codec/compatibility gates remain
...
*End RFC v7.7. Architecture approved; protocol approved with conditions; Steps 1–2.5 go. **Master RFC FROZEN** — core wire via P1–P16; provisional beta.2 browser schema via P17; product scope via the Web Console RFC.*

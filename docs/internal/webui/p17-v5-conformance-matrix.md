...
| Real rendered reference evidence | Render evidence v2 binds 144 matrix renders + 5 supplemental renders to tracked-clean candidate `382007f77687442b2e8ea0912dcfb705dba953cb`, Web build `sha256:853edc81c345c5a06e105110c4f3cb0fabcfc79f0e73912f05fa636b56da624e`, and asset manifest `sha256:154fe609eb6bea194e6b241e1b3f7394ffcecbed7cf70352faf44b9eef08a9e6`. It rejects relevant source drift and requires downstream review records to name that exact candidate. All 149 screenshot bytes are retained under `artifacts/web-reference/382007f77687/`; 36 representative renders report zero serious/critical axe findings. | **PASS (local)** for current exact-source rendering and automated checks; human product/content approval remains open |
...
| Native Simplified-Chinese content review | Complete key parity and checked reviewer materials cover 168 projected + 188 static = 356 keys. The verifier requires one independent native `content-ux` and one `technical-safety` approval bound to exact render candidate `382007f7` and its catalog/render hashes. | **EXTERNAL BLOCKER** — both reviewer attestations remain absent |
| Human reference approval | Nine screen specifications, the exact-source 149-render gallery, and checked no-evidence approval materials exist. The verifier binds product-owner review to exact v2 render candidate `382007f7` rather than accepting an ancestor. | **EXTERNAL BLOCKER** — manifest remains `draft-unapproved` and no product-owner approval exists |
...
TASKFLOW_WEB_REFERENCE_OUTPUT_ROOT=artifacts/web-reference/382007f77687 pnpm render:web-reference
```

...
records as historical-only while separately validating five current-source
Safari scoped records against `c0454001`/`f6dfc801`, including an independent
normal/private two-cookie-jar revocation path. The retained 30-sample
benchmark rerun is `structural-pass`; its human and JSON reports also disclose
the preceding large-list timeout instead of treating the immediate successful

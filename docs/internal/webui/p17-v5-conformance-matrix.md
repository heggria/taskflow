...
Status: candidate evidence ledger; **the current source is not yet rerendered,
not Conforming, and not wire-frozen**.
...
| Executable codecs | Every row owns TypeBox params/query/body/success schemas; producer DTOs are closed; committed vectors cover the 13 approved additive roots, all command/action/outcome/page-cursor/SSE branches, all 29 endpoint samples, every error code, and recursively validate 39,707 branches across 2,225 nested union occurrences. `scripts/test-web-packaged-compatibility.mjs` rejects identical asset or commit identities, waits for a successful bootstrap response, and cross-runs real browser assets plus gateway/control modules in both directions. Immutable candidates `7e555c7d7f39109ee89a502f0d818cc34f1ce7fa` and `aa34369a5958ce34bbdad3eab973434a001d0738` pass old-client/new-server and new-client/old-server with Web v1 bootstrap and zero console/page errors; the schema-v2 report is checked in at `artifacts/web-compat/7e555c7d-to-aa34369a/report.json`. The evidence checker rejects a non-descendant evidence tip or production/benchmark source drift after the new build. | **PASS (local)** for current codecs and the latest older immutable pair; reviewed-tip rerun remains a freeze gate |
...
| Real rendered reference evidence | The existing 144 matrix renders + 5 supplemental renders remain valid only for the older immutable source. Render evidence v2 now requires an exact tracked-clean Git commit, Web build id and asset-manifest digest, rejects relevant source drift, and requires downstream review records to name that exact candidate. | **PENDING** — current 356-key source must be rebuilt and rerendered |
...
| Artifact and Receipt export delivery | Current Receipt reachability, pre-header content-addressed artifact snapshot digest/length, source-tamper rejection, all redaction classes, projected direct/acknowledgement/blocked disclosure, explicit sensitive acknowledgement header, secret-action absence, success redaction header, listener-wide pessimistic 100-MiB in-flight byte permit, no second full-body copy, MIME and disposition policy, Range rejection, backpressure/abort/deadline controls, and prior exact 100-MiB transport evidence. Receipt JSON export re-reads all byte-budgeted manifest pages and rejects identity or ordering drift before deterministic export. | **PASS (local)** for new schema/transport/gateway fixtures; current packaged UI interaction rerun pending |
...
| Performance and large-data behavior | The production builder enforces the 220-KiB-gzip Simple-shell ceiling; CSP-compatible task segmentation and graph paging are exercised by `scripts/bench-web.mjs` over the deterministic 100-project/10,000-run/2,000-node fixture. Immutable commit `aa34369a5958ce34bbdad3eab973434a001d0738`, source digest `sha256:f05e3aefb64dd3a5d115e4c975883ecf16e5830f7c9f225fb07962e317a40db4`, has a checked-in schema-v4 Node 24.18.0/M3 Pro 30-sample clean-source `structural-pass`: 209,803-byte Simple shell; cold Home p95 511.8 ms; warm Home p95 887.4 ms; cached Pro p95 55.7 ms; committed event→painted detail p95 344.6 ms (commit→receipt 123.8 ms, receipt→paint 281.4 ms diagnostics); 13-ms list response; CLS 0.0616; all 10,000 task rows and 2,000 graph nodes; zero CSP/runtime-style findings. JSON and human summaries retain owner-ready and launch-to-useful p95 at 4.977 s and 5.556 s rather than hiding startup. The report verifies `git.dirty:false`, exact primary Node version, AC power and no recorded thermal/performance warning. Load rose from 5.54/6.51/7.58 to 9.70/8.09/7.94. | **PARTIAL** — the immutable 30-sample implementation/methodology is green locally, but the M3 Pro run is non-canonical and its 344.6-ms event-to-visible p95 is above the 250-ms canonical budget; it remains informational. The required unloaded canonical M2/Node 24 run and percentile sign-off remain absent |
| Packaged browser path | Built packages → CLI → daemon → multi-project WebGateway → browser; independent cross-port listeners, SSE, live/stopped polling projection equivalence, every current GET polling query surface, live cancel, restart-safe contextual approval, downstream execution and Receipt, private-checkpoint non-reachability, Receipt/artifact plus deterministic JSON export, zero-write/zero-provider replay, Pro graph/timeline/evidence, React Aria tabs/disclosure/alert-dialog/listbox and single-selection relations, Simple → Pro focus restoration, keyboard selection, graph arrow-navigation/inspector parity, modal focus containment/Escape/trigger restoration, settings switch semantics, current-session logout isolation, listener-wide revoke-all invalidation of an independent peer session, 320px and Chromium axe | **PASS (local)** for the exact `aa34369a` representative complete path on Chromium, Firefox, WebKit and native Chrome plus current-source native Safari scoped mutation smokes; assistive-technology matrices remain |
| Packaged browser engines | Exact candidate `aa34369a` passes Chromium 149.0.7827.55, Firefox 151.0 and WebKit 26.5 from the same source state, each on its first matrix attempt with zero application console/page errors, CSP violations, runtime style insertions, final inline styles or runtime style elements. All three pass Receipt export, zero-write/zero-provider replay, graph-listbox keyboard parity, current-session logout and listener-wide peer-session revocation. Firefox records six explicitly classified EventSource navigation-interruption diagnostics; they are transport diagnostics, not application console failures. Native Google Chrome 150.0.7871.184 passes the same complete packaged path and Chromium axe checks. Native Safari 26.3 on macOS 26.3 has independently recorded packaged read/keyboard and five scoped mutation smokes: allow/revoke-all, reject/current-session and cancel rerun on evidence tip `60002c34`; HTTP-actor peer invalidation rerun on `e19dfd45`; and normal/private Safari two-cookie-jar revocation on `d6ac8877`. All three evidence tips retain byte-identical `aa34369a` candidate source. Cancel proves the live node settles as stopped instead of remaining active. The HTTP-actor record proves a Safari target transitions to the localized session-ended screen after an independent authenticated P17 client revokes all sessions. The later native two-cookie-jar record proves the same listener-wide revocation across independently authenticated normal and unlocked-private Safari windows. Every passing hold verifies listener close. | **PARTIAL** — the exact-source Playwright three-engine matrix, native Chrome and native Safari scoped mutation smokes pass; Edge, VoiceOver and the remaining assistive-technology/manual matrices remain |
...
| Native Simplified-Chinese content review | Complete key parity and checked reviewer materials now cover 168 projected + 188 static = 356 keys. The verifier requires one independent native `content-ux` and one `technical-safety` approval bound to the exact render candidate and catalog/render hashes. | **EXTERNAL BLOCKER** — current candidate rerender plus both reviewer attestations absent |
| Human reference approval | Nine screen specifications and checked no-evidence approval materials exist. The verifier binds product-owner review to the exact v2 render candidate rather than accepting an ancestor. | **EXTERNAL BLOCKER** — current candidate rerender absent and manifest remains `draft-unapproved` |

## Latest local verification

The following commands exited 0 on 2026-07-25:

```text
pnpm verify:web-candidate  # evidence-only descendant of source aa34369a
...
npx --yes node@24.18.0 scripts/test-web-packaged-compatibility.mjs --old-root <7e555c7d-build> --new-root <aa34369a-build> --output artifacts/web-compat/7e555c7d-to-aa34369a/report.json
pnpm test
pnpm test:pack
...
- packaged browser: the approval-enhanced source state passes Chromium
  ...
  same complete path and axe checks. Native Safari 26.3 on macOS 26.3 has
  separately scoped read/keyboard, allow/revoke-all,
  reject/current-session, cancel, HTTP-actor peer-invalidation and native
  two-cookie-jar revocation smokes recorded in `native-safari-smoke-v1.json`
  and the five mutation records. All explicitly exclude VoiceOver and
  automated console/CSP evidence. The earlier peer-invalidation record uses
  native Safari as the revoked target and an independent P17 HTTP session as
  the mutation actor; the later record uses independently authenticated normal
  and unlocked-private Safari windows. Approval, rejection and cancel were
  rerun on evidence tip `60002c34`; HTTP-actor peer invalidation on `e19dfd45`;
  and native two-cookie-jar revocation on `d6ac8877`. All three tips bind the
  byte-identical `aa34369a` candidate source.
...
reduce the largest minified chunk to 191.20 kB and the entry chunk to 158.88
kB. The production asset builder measures the modulepreloaded Simple shell at
209,803 gzip bytes (204.89 KiB) and fails above the RFC's 220-KiB budget; the

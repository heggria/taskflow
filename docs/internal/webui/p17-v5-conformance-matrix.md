...
| Performance and large-data behavior | The production builder enforces the 220-KiB-gzip Simple-shell ceiling; CSP-compatible task segmentation and graph paging are exercised by `scripts/bench-web.mjs` over the deterministic 100-project/10,000-run/2,000-node fixture. Immutable commit `d8df9c650b68e4c0854aaad03e6da3ce0dc3352f`, source digest `sha256:648b778e4e316be5f93e4ef2b68c07ba45bea4d0d2731a9dc2d471ec3b18c0b3`, has a checked-in schema-v4 Node 24.18.0/M3 Pro 30-sample clean-source `structural-pass`: 209,367-byte Simple shell; cold Home p95 1,268.1 ms; warm Home p95 904.7 ms; cached Pro p95 30.7 ms; committed event→painted detail p95 151.1 ms (commit→receipt 76.9 ms, receipt→paint 95.0 ms diagnostics); 8.6-ms list response; CLS 0.0784; all 10,000 task rows and 2,000 graph nodes; zero CSP/runtime-style findings. JSON and human summaries retain owner-ready and launch-to-useful p95 at 25.700 s and 27.175 s rather than hiding startup. The report verifies `git.dirty:false`, exact primary Node version, AC power and no recorded thermal/performance warning, but also records high pre-run load 8.05/8.69/8.62 and large owner-launch outliers. | **PARTIAL** — the immutable 30-sample implementation/methodology is green locally, but the high-load M3 Pro run is non-canonical and informational; the required unloaded canonical M2/Node 24 run and percentile sign-off remain absent |
| Packaged browser path | Built packages → CLI → daemon → multi-project WebGateway → browser; independent cross-port listeners, SSE, live/stopped polling projection equivalence, every current GET polling query surface, live cancel, restart-safe contextual approval, downstream execution and Receipt, private-checkpoint non-reachability, Receipt/artifact plus deterministic JSON export, zero-write/zero-provider replay, Pro graph/timeline/evidence, React Aria tabs/disclosure/alert-dialog/listbox and single-selection relations, Simple → Pro focus restoration, keyboard selection, graph arrow-navigation/inspector parity, modal focus containment/Escape/trigger restoration, settings switch semantics, current-session logout isolation, listener-wide revoke-all invalidation of an independent peer session, 320px and Chromium axe | **PASS (local)** for the representative complete path on Chromium, Firefox, WebKit and native Chrome; native Safari separately covers read/keyboard, allow/revoke-all and reject/current-session smokes, while cancel/peer-session and assistive-technology matrices remain |
| Packaged browser engines | The approval-enhanced packaged candidate passes Chromium 149.0.7827.55, Firefox 151.0 and WebKit 26.5 from the same source state, each on its first matrix attempt with zero application console/page errors, CSP violations, runtime style insertions, final inline styles or runtime style elements. All three pass Receipt export, zero-write/zero-provider replay, graph-listbox keyboard parity, current-session logout and listener-wide peer-session revocation. Firefox records six explicitly classified EventSource navigation-interruption diagnostics; they are transport diagnostics, not application console failures. Native Google Chrome 150.0.7871.184 passes the same complete packaged path and Chromium axe checks. Native Safari 26.3 on macOS 26.3 has independently recorded packaged read/keyboard and two mutation smokes on the immutable `d8df9c65` source: one activates Allow from keyboard focus, reaches completed 3/3 with partial verification and result `published`, then applies listener-wide revocation; the other reaches Reject through native Tab focus, proves the approval node is blocked while downstream stays pending, then applies current-session logout. Both render the appropriate localized signed-out screen and verify listener close. | **PARTIAL** — Playwright's three-engine matrix, native Chrome and the scoped native Safari smokes pass locally; Edge, Safari cancel/peer-session cases, VoiceOver and the remaining assistive-technology/manual matrices remain |
...
npx --yes node@24.18.0 scripts/test-web-packaged-compatibility.mjs --old-root <83021958-build> --new-root <d8df9c65-build> --output artifacts/web-compat/83021958-to-d8df9c65/report.json
pnpm test
pnpm test:pack
...
- packaged browser: the approval-enhanced source state passes Chromium
  ...
  the separately scoped read/keyboard, approval/revoke-all and
  reject/current-session smokes recorded
  in `native-safari-smoke-v1.json` and
  the two mutation records; all explicitly exclude VoiceOver, and the mutation
  records do not claim cancel/peer-session coverage or automated console/CSP
  evidence.
...
209,367 gzip bytes (204.46 KiB) and fails above the RFC's 220-KiB budget; the
...
performance evidence, native Edge/remaining-Safari/AT review, reviewed-tip

...
| Performance and large-data behavior | The production builder enforces the 220-KiB-gzip Simple-shell ceiling; CSP-compatible task segmentation and graph paging are exercised by `scripts/bench-web.mjs` over the deterministic 100-project/10,000-run/2,000-node fixture. Current immutable build `130e1a3f7c0bf991c96d6e0192a2517be43ff827`, source digest `sha256:28142aa423b56628a67f9f38d2dbccf7ba985750a1ccca1d0cb4bc9e050a1682`, has a checked-in schema-v4 Node 24.18.0/M3 Pro 30-sample clean-source `structural-pass`: 211,426-byte Simple shell; cold Home p95 405.7 ms; warm Home p95 453.3 ms; cached Pro p95 24.4 ms; committed event→painted detail p95 291.8 ms (commit→Receipt 66.4 ms, Receipt→paint 225.0 ms diagnostics); 3.6-ms list response; CLS 0.0397; all 10,000 task rows and 2,000 graph nodes; zero CSP/runtime-style findings. JSON and human summaries retain owner-ready and launch-to-useful p95 at 3.075 s and 3.509 s rather than hiding startup. The report verifies `git.dirty:false`, exact primary Node version, AC power and no recorded thermal/performance warning. One preliminary large-list run timed out; an immediate smoke and the retained 30-sample rerun completed. | **PARTIAL** — structural, bundle, CSP and layout checks pass, but this is a non-canonical M3 Pro run and event→visible p95 is 291.8 ms against the canonical 250-ms budget. The required unloaded canonical M2/Node 24 run and percentile sign-off remain absent |
...
| Packaged browser engines | Current immutable packaged/evidence build `130e1a3f7c0bf991c96d6e0192a2517be43ff827` passes Chromium 149.0.7827.55, Firefox 151.0, WebKit 26.5 and native Chrome 150.0.7871.184 from byte-identical production assets, each on its first attempt with zero application console/page errors, CSP violations, runtime style insertions, final inline styles or horizontal overflow. The schema-v4 report binds the exact two-file executable browser harness. All four lanes pass Receipt export, zero-write/zero-provider replay, graph-listbox keyboard parity, current-session logout, listener-wide peer-session revocation and the sensitive-artifact decline/acknowledgement contract. Firefox records six explicitly classified EventSource navigation-interruption diagnostics; they are transport diagnostics, not application console failures. Chromium and native Chrome pass the same axe checks. Native Safari 26.3 now has four current-source scoped records bound to `c0454001`: read/keyboard/graph/evidence, Allow plus revoke-all, Reject plus current-session logout, and live cancel. The checker proves no production-source drift from immutable build `130e1a3f` and rejects record/source identity drift. | **PARTIAL** — the current exact-source Playwright three-engine matrix, native Chrome and scoped native Safari paths pass; current-source Safari independent peer/two-cookie invalidation, Edge, VoiceOver and the remaining assistive-technology/manual matrices remain |
...
records as historical-only while separately validating four current-source
Safari scoped records against `c0454001`. The retained 30-sample benchmark
rerun is `structural-pass`; its human and JSON reports also disclose the
preceding large-list timeout instead of treating the immediate successful
smoke and rerun as if no failed attempt occurred.
...
- packaged browser: the approval-enhanced source state passes Chromium
  ...
  listener-wide revoke-all scoped paths. Those records do not claim
  VoiceOver, console/CSP/axe automation, an independent peer/two-cookie
  current-source session path or human approval.

...
> Status: **Active** · Draft v1 2026-07-08 · **Implementation progress updated 2026-07-09**
...
> | **S2** | 🟡 切片 | `exec/{step,driver}` 支持 **agent\|script\|map\|parallel**；默认 OFF；其余 kind 仍走 imperative path |
...
>
> North-star 口号已改为 **compiled · resumable · incremental · replayable-for-what-if**。
...
7. **DSL 是平行前端（S4 ⬜）**：`.tf.ts → build → Taskflow → FlowIR`；新包 `taskflow-dsl`。
...
→ 本 RFC 拍板：FlowIR **是**规范执行/编译真源方向（Q2=B），用绞杀守住可发布性（Q7）。S0–S3 已部分兑现；S4–S5 继续。
...
| **S2** 🟡 | 建 `exec/{driver,step}`；**已迁** agent→script→map→parallel；剩余 reduce/gate/loop/tournament/approval/flow | 已支持 kind 的 parity 测试；默认 OFF | 每 kind 绿了 flip 一次 | Q7 |
...
1. **`build` AST transform 实现选型**（S4）：tsc transformer API / Babel / 自研轻量解析（DSL RFC v2 §C）— **仍待定**。
...
4. **绞杀开关的默认策略**：**已决（S2 切片）** — 默认 OFF；`RuntimeDeps.eventKernel` 或 `PI_TASKFLOW_EVENT_KERNEL=1|true`；显式 `false` 覆盖 env。
5. **S3 与 S4 并行 vs 串行**：S3 已落地；**S4 为下一主线**（与 S2 剩余 kind 可并行）。

...
| **确定性重放 / replayable-for-what-if** | 事件溯源 + fold | `trace` + `replayRun` / `taskflow_replay` / `/tf replay`（阈值/budget what-if，零 token） | ✅ 0.2.0 S3 已落地 |
| 增量重算（只重算受影响部分） | Bazel/Nix | content-addressed cache + why-stale + recompute | ✅ 已落地 |
| 写一次编译多端 | Mitosis | 一份 DSL → pi/codex/claude/opencode/grok 五 host | ✅ 已落地 |

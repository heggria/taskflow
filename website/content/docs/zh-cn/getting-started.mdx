...
<Callout type="info">
  本指南覆盖稳定的 0.2.x 宿主安装路径。当前 **0.3 Trusted Effects candidate** 请先阅读 [Trusted Effects 总览](/zh-cn/docs/trusted-effects)，并从仓库 checkout 运行；0.3 尚未发布，也不是 GA。
</Callout>
...
<Tabs groupId="host" items={['Pi', 'Codex', 'Claude', 'OpenCode', 'Grok', 'Hermes']}>
  ...
  <Tab value="Codex">
    ...
    # 让 Codex 通过 taskflow_run 调用 hello define（或已保存名称）。
    ```
  </Tab>
  <Tab value="Claude">
    ```bash title="在 Claude Code 上运行"
    # 让 Claude 通过 taskflow_run 调用 hello define（或已保存名称）。
    ```
  </Tab>
  <Tab value="OpenCode">
    ```bash title="在 OpenCode 上运行"
    # 让 OpenCode 通过 taskflow_run 调用 hello define（或已保存名称）。
    ```
  </Tab>
  <Tab value="Grok">
    ```bash title="在 Grok Build 上运行"
    # 需要先完成下方「安装」里的自定义 profile 配置。
    # 让 Grok 通过 MCP 调用 taskflow_verify / taskflow_run。
    ```
  </Tab>
  <Tab value="Hermes">
    ```bash title="在 Hermes Agent 上运行"
    # 让 Hermes 通过 MCP 调用 taskflow_verify / taskflow_run。
    ```
  </Tab>
</Tabs>
...
<Callout type="tip">
  **0.2.7 习惯：** 对任何非平凡 flow，先用与 `run` 相同的 `args` 调 **`taskflow_plan` / `/tf plan`**。免费、绑定 typed 参数，并在花钱前给出 worst-case agent 调用上界。详见 [0.2.7 新能力](/zh-cn/docs/blog/plan-before-spend-0.2.7)。
</Callout>
...
<Callout type="warn">
  所有 taskflow 包都要求 **Node.js 22.19.0 或更高版本**。
</Callout>
...
<Tabs groupId="host" items={['Pi', 'Codex', 'Claude', 'OpenCode', 'Grok', 'Hermes']}>
  ...
  <Tab value="Grok">
    ...
    ```
    只读阶段与可写或未声明 tools 的阶段都会在对应自定义 profile 未配置时 fail closed。
  </Tab>
  <Tab value="Hermes">

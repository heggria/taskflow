...
评分门控遵循与普通 gate 相同的“宽松解析但 fail-closed”原则：评分器失败会回落到裁判或 task，但完全无法解析的 gate 输出会被当作 `BLOCK`（issue #54）。配置错误（如格式错误的 `scorers`、未解析的 `target`）仍保持 fail-open，并附带警告。
...
**完全无法解析时 fail-closed。** 如果 gate 既没有可识别的 JSON 裁决，也没有 `VERDICT:` 标记，运行时会回退到 `BLOCK`（issue #54）——一个无法给出裁决的 gate 不能被认为是安全的。而明确的非阻塞 JSON 裁决（例如 `{"verdict": "no issues found"}`）仍是语义上的 `PASS`，不是歧义。

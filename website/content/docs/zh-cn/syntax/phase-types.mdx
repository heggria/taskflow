---
title: 阶段类型
description: taskflow 的十二个构建块，以及何时选用各自。
---

一个 taskflow 是由**阶段**组成的有向图。每个阶段是一个工作单元 —— 一次隔离的子代理调用、一次 shell 命令、或一个控制决策 —— 运行时通过 `dependsOn` 把它们连起来。本页按你实际会遇到它们的顺序，逐一讲解全部十二种阶段类型：从你 80% 场景都会用的那个开始，再随着问题变复杂逐步加上能力。

你可能在想：*我得先把十二种全学会才能干活吗？* 不用。大多数 flow 只用三四种类型就建起来了。其余几种是给特定场景准备的 —— 人工审批、迭代精修、竞争草稿 —— 在你撞上那些场景之前，完全可以无视它们。
...
| "保留第一个成功的方法" | `race` | 第一个成功的分支获胜；硬失败不算。 |
| "执行规划器生成的小片段" | `expand` | 嵌套隔离或 graft 提升到父流程。 |
...
<Callout type="tip">
  **宽松解析，fail-closed 回退。** taskflow 会尽力提取裁决——JSON 字段、`VERDICT:` 标记、自然语言词汇。如果输出完全无法解析，gate 会 fail-closed（`BLOCK`，issue #54）。一个无法给出裁决的 gate 不能被认为是安全的。配置或解析错误（如未解析的 `score.target`、格式错误的 `scorers`）仍保持 fail-open，并附带警告。
</Callout>
...
- **`timeoutMs`（0.2.7，可选）** → 等待人工的最长时间（`>= 1000` ms）。省略则永等（旧行为）。
- **`onExpire`（0.2.7）** → 超时后 `reject`（默认）| `fail` | `approve`。`approve` 是显式脚枪。

<Callout type="warn">
  approval 只在交互式宿主（Pi）中有效。在 CI 或分离运行中，`approval` 阶段会**自动 reject** —— 所以别把它放在你会无人值守运行的 flow 上。agent 墙钟 `timeout` 与 `cache.scope: "cross-run"` 在 approval 上不支持；HITL 等待请用 **`timeoutMs`**。
</Callout>
...
当多种路径都能回答同一问题、且**延迟**比等齐所有分支更重要时，用 `race`。各分支同时启动，**第一个成功完成**的输出成为阶段结果（快速硬失败**不会**赢）。`parallel` 会等全部；`tournament` 会等齐后由裁判比质量。

```json title="race — 先成功者胜"
{
  "id": "quick",
  "type": "race",
  "branches": [
    { "task": "用简短启发式作答…", "agent": "executor" },
    { "task": "查一下文档再作答…", "agent": "researcher" }
  ],
  "final": true
}
```
...
<Callout type="info">
  **`cancelLosers`（默认 true）：** 第一个**成功**后，其余分支收到 `AbortSignal`（尽力而为）。阶段 `usage` 汇总所有分支以便预算诚实。设 `false` 可让败者跑完。
</Callout>

## 动态片段：`expand`

`expand` 从 `def` 运行一段片段 Taskflow（内联对象、phases 数组，或 `{steps.plan.json}`）。两种模式：

| `expandMode` | 行为 |
|---|---|
| `nested`（默认） | 隔离子流（类似 `flow`+`def`）；子阶段 id 不进入父 run |
| `graft` | 成功后把子阶段状态提升到父 run，id 为 `<expandId>-<childId>` |

```json title="expand — graft 提升"
{
  "id": "grow",
  "type": "expand",
  "expandMode": "graft",
...
可选 `maxNodes` 限制片段规模（默认 50，硬上限 100）。动态校验与 `flow{def}` 安全帽一致。

<Callout type="warn">
  **安全帽。** 与 `flow{def}` 一样，`expand` 强制执行：最多 100 个阶段、200 个 map 项、16 并发、5 层嵌套、不允许 `script`，以及 `cwd` 必须留在运行目录内。`graft` 模式下，子阶段 id 会被提升为 `\u003cexpandId\u003e-\u003cchildId\u003e`，片段内部的 `{steps.X}` 引用也会以同样前缀重写。
</Callout>
...
| `type` | `string` | `"agent"` | 十二种阶段类型之一。 |
| `agent` | `string` | 第一个发现的 agent | agent 名称。必须用连字符。 |
| `task` | `string` | — | 任务 prompt。支持插值。对 `agent`/`map`/`reduce`/`loop`/`tournament` 必需（除非用 `branches` 或 `taskFile`）。 |
| `taskFile` | `string` | — | 加载期 include。相对于 flow 定义文件目录的字面路径。可信加载器（`defineFile` / 已保存 flow）把 UTF-8 正文写入 `task` 并删除 `taskFile`，之后 validate / interpolate / cache / FlowIR 只看见 `task`。与 `task` 互斥。路径**不**插值。禁止出现在 inline `define` 和生成的 `flow{def}` / expand 上。上限：256 KiB。`taskflow-dsl` 编译/`check` **不**读这个文件。 |
...
<Callout type="info">
  **公共字段的校验规则。**
  ...
  - `cache.scope: "cross-run"` 在 `gate`、`approval`、`loop`、`tournament`、`script`、`race` 和 `expand` 上禁用。
...
¹ `gate` 在不用 `score` 时需要 `task`。² `flow` 需要 `use` 或 `def` 恰好其一。³ `tournament` 在不用 `branches` 时需要 `task`。⁴ `race` 至少 2 条 `branches`。`expand` 另有 `expandMode`（`nested`\|`graft`）与 `maxNodes`。
...
<details>
<summary><code>race</code> — <code>branches</code>、<code>cancelLosers</code></summary>

- **`branches`**（`{task, agent?}[]`，必需）：至少两条竞争分支。第一个成功的分支获胜；快速硬失败不算赢。
- **`cancelLosers`**（`boolean`，默认 `true`）：第一个成功后，向剩余分支发送 `AbortSignal`（尽力而为）。阶段 `usage` 汇总所有分支以便预算诚实。设 `false` 让败者跑完。
- **`task`**：忽略 —— 用 `branches` 代替。

</details>

<details>
<summary><code>expand</code> — <code>def</code>、<code>expandMode</code>、<code>maxNodes</code></summary>

- **`def`**（必需）：内联或运行时生成的片段定义。字符串会被插值再 JSON 解析；对象直接使用。
- **`expandMode`**（`"nested"` | `"graft"`，默认 `"nested"`）：`"nested"` 像 `flow{def}` 一样隔离片段；`"graft"` 把子阶段状态提升到父 run，id 为 `\u003cexpandId\u003e-\u003cchildId\u003e`。
- **`maxNodes`**（`number`，默认 `50`，硬上限 `100`）：限制片段规模。动态校验与 `flow{def}` 安全帽一致。
- **`task`**：忽略。
...
| 错误 | 原因 | 修复 |
|---|---|---|
| `Phase 'x' (agent) requires 'task'` | `task` 缺失或为空。 | 加一个 `task` 字符串或 `taskFile`。 |
| `'task' and 'taskFile' are mutually exclusive` | 两个字段同时存在。 | 只保留其中一个。 |
| `TF_TASKFILE_NO_PROVENANCE` | 在 inline `define` 上使用了 `taskFile`。 | 改用 `defineFile` 或已保存 flow。 |
| `TF_DYNAMIC_RESOURCE_FORBIDDEN` | 在生成的子流上使用了 `taskFile`。 | 把 prompt 写成 `task`。 |

...
| `taskFile` | `string` | — | 加载期 include。相对于 flow 定义文件目录的字面路径。可信加载器（`defineFile` / 已保存 flow）把 UTF-8 正文写入 `task` 并删除 `taskFile`，之后 validate / interpolate / cache / FlowIR 只看见 `task`。与 `task` 互斥。路径**不**插值。禁止出现在 inline `define` 和生成的 `flow{def}` / expand 上。上限：256 KiB。 |

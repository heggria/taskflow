---
title: 阶段
description: taskflow 的十二个构建块，以及如何在它们之间做选择。
---

**阶段**是 taskflow 中的一个工作单元。每个阶段作为一次隔离的子代理调用、一条 shell 命令或一个控制决策运行，运行时通过 `dependsOn` 把它们连成一张 DAG。

理解这十二种阶段类型最好的方式，是看同一个问题如何一步步演化。想象你在做一个审查 pull request 的工具。
...
如果 gate 的输出完全无法解析——没有 JSON、没有 `VERDICT:` 标记、没有可识别的裁决词——taskflow 会 fail-closed，把它当作 `BLOCK`（issue #54）。一个无法给出裁决的 gate 不能被认为是安全的。而明确的非阻塞 JSON 裁决（例如 `{"verdict": "no issues found"}`）是语义上的 `PASS`，不是歧义。
...
多种路径都能回答同一问题、且你要**最快成功**的结果时用（不是质量裁判）。快速硬失败**不会**胜出。
...
```json title="race-example.json"
{
  ...
  "type": "race",
  "branches": [
    { "task": "启发式作答…", "agent": "executor" },
    { "task": "仔细作答…", "agent": "researcher" }
  ]
}
```

### `expand` —— 运行动态片段（nested / graft）

规划器产出迷你 flow JSON、由运行时执行时用。`expandMode: "graft"` 会把子阶段状态提升到父 run，id 为 `<expandId>-<childId>`。
...
| "保留最先成功的路径" | `race` | 延迟优先于等齐/裁判；硬失败不胜出。 |
| "跑规划器吐出的片段" | `expand` | 隔离子流或 graft 提升到父 run。 |

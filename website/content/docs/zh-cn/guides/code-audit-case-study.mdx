...
<Callout type="info">
  taskflow 对 gate 的输出做宽松解析，但完全无法解析时 fail-closed。如果 gate 输出既无 JSON、也无 `VERDICT:` 标记和可识别的裁决词，运行时会把它当作 `BLOCK`（issue #54）——一个无法给出裁决的 gate 不能被认为是安全的。如果你希望合并 gate 的最终决定由人来做，可以在下游加一个 `approval` 阶段。
</Callout>
...
`budget: { maxUSD: 3.0 }` 是基于已上报用量的运行级止损线。观察到累计成本越线后，运行时不再准入新调用并跳过剩余阶段。已经活动的调用仍可能超支，因此它限制的是失控花费，并不保证账单最多 3 美元。请按仓库调整，并记住 gate 在报告*之前*运行，所以一个 `BLOCK` 判约会省下报告的 token。
...
<Cards>
  <Card title="阶段类型" href="/zh-cn/docs/concepts/phases">
    十二种构建块——包括这里用到的 `map`、`parallel`、`reduce` 和 `gate`。
  </Card>
  ...
  <Card title="预算" href="/zh-cn/docs/syntax/budget">
    基于已上报用量的运行级止损线，以及 `BLOCK` 判决如何省下下游 token。
  </Card>

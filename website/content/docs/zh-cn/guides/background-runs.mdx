...
1. **FlowIR Compilation (M1 / 0.2.0 S0)** - `compileTaskflowToIR(def)` returns `TaskflowIR` with content-addressed `hash` (`ir:<64-hex>`), `meta.declaredDeps`, and `usedFallbackHash: false` for well-formed flows (genuine compiler).
...
interface TaskflowIR {
  ...
  errors: CompileError[];         // Hard errors
  usedFallbackHash: boolean;      // false when hash is IR-canonical (`ir:<64-hex>`)
}
```
...
As of 0.2.0 S0, `usedFallbackHash` is **false** when the genuine compiler content-addresses FlowIR (`hash` is `ir:<64-hex>` via `hashFlowIR`). It is true only on hash failure or empty IR. A separate `flowDefHash` may still appear in older cache-key tiers.

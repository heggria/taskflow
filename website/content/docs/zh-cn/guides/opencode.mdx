...
| 阶段工具 | 权限策略 | 含义 |
|---|---|---|
| 只读（无 `write`/`edit`/`bash`） | `--pure` + 注入默认拒绝策略 | 禁用外部插件；仅允许已知的读取/列举/搜索内置工具，因此继承的自定义与 MCP 工具也会被拒绝。 |
...
子进程只继承平台、代理/CA、OpenCode 配置与受支持 provider 变量；无关的父进程 secret 会被移除。确需额外任务变量时，通过逗号分隔的 `PI_TASKFLOW_CHILD_ENV_ALLOW` 显式列出变量名。

...
| 阶段工具 | 权限模式 | 含义 |
|---|---|---|
| 只读或未声明；不请求变更类/未知工具 | `--allowedTools Read,Grep,Glob,WebFetch,WebSearch` | 变更类工具被直接拒绝。注意没有只读 shell——不会授予 `Bash`。 |
| 请求变更类或未知工具 | 默认拒绝 | 仅对可信 flow 设置 `PI_TASKFLOW_CLAUDE_UNSAFE_BYPASS=1`，显式启用 `bypassPermissions`。 |

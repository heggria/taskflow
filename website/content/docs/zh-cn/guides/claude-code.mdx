...
[Claude Code](https://claude.com/product/claude-code) 是 taskflow 的天然宿主：它本身就以 subagent 的方式思考，而 taskflow 给这些 subagent 提供了一个声明式图来运行。插件注册 MCP server 与路由 skill；协议层不依赖 MCP SDK。
...
<Steps>
  ...
  <Step>
    **不依赖 MCP SDK。** 协议层直接通过 stdio 讲 JSON-RPC 2.0。交付包仍依赖 taskflow 内部包，core peer 依赖 `typebox`。
  </Step>
</Steps>
...
Taskflow 要求 Claude Code **2.1.169 或更高版本**以使用 `--safe-mode`。旧版 CLI 会因未知参数而 fail closed，请先升级再使用此适配器。

| 阶段工具 | 权限模式 | 含义 |
|---|---|---|
| 只读或未声明；不请求变更类/未知工具 | 对齐的 `--tools` + `--allowedTools` 列表 | 内置工具既被限制又被预批准；显式列表不会被放宽。`--safe-mode` 禁用非托管自定义项，磁盘 settings source 和非托管 hooks 被关闭；管理员托管策略 hooks 仍可能运行。注意没有只读 shell——不会授予 `Bash`。 |
| 请求已知变更类工具 | 默认拒绝 | 对可信 flow，`PI_TASKFLOW_CLAUDE_UNSAFE_BYPASS=1` 会启用 `bypassPermissions` 并保留收窄的 `--tools`；未知名称始终 fail closed。 |

<Callout type="warn">
  unsafe 模式必须由操作者显式开启，不是默认值。即使开启，也应优先使用一次性 worktree（`cwd: "worktree"`）。Claude 子进程仅继承平台/运行时、代理/CA 与 Claude 支持的 provider 环境变量；无关的应用密钥会被移除。
</Callout>

## 长时运行的 flow

`taskflow_run` 默认同步。长 flow 可设置 `mode: "background"`；MCP 调用会立即返回持久化 `runId`。使用 `taskflow_runs` 列出、查看、等待或取消该 worker。

...
<Callout type="info">
  这些模板在 Pi（`/tf run`）和 Codex / Claude Code / OpenCode / Grok Build / Hermes Agent（`taskflow_run`）上运行方式完全一致。宿主特定安装见 [Pi](/zh-cn/docs/guides/pi)、[Codex](/zh-cn/docs/guides/codex)、[Claude Code](/zh-cn/docs/guides/claude-code)、[OpenCode](/zh-cn/docs/guides/opencode)、[Grok Build](/zh-cn/docs/guides/grok-build) 或 [Hermes Agent](/zh-cn/docs/guides/hermes) 指南。
</Callout>
...
把任意模板保存为项目里的 `.json` 文件，然后在运行前先 **plan**（并可再 verify）：

```bash title="运行前先 plan（0.2.7）"
/tf plan <name> '{"dir":"src"}'   # 绑定 args + 预算上界，零 token
/tf verify <name>                 # 结构检查
```

<Callout type="info">
  **0.2.7 入门模板**也在 monorepo [`examples/templates/`](https://github.com/heggria/taskflow/tree/main/examples/templates)：`plan-first.json`（先 plan 习惯）与 `background-with-hooks.json`（file hooks + 审批超时）。详见 [0.2.7 发布说明](/zh-cn/docs/blog/plan-before-spend-0.2.7)。
</Callout>
...
<Tabs groupId="host" items={['Pi', 'MCP hosts']}>
  ...
  <Tab value="MCP hosts">
    ```bash title="在 Codex / Claude / OpenCode / Grok / Hermes 上运行"
...
- **`budget`**——基于已上报用量的 run 级止损线。`BLOCK` 裁决或观察到预算越线后不再启动下游调用；已经活动的调用（包括并发 `race` 分支）仍可能超支。

...
| `maxKeptRuns` | `100` | 保留的非活动运行（`completed`、`failed`、`paused`、`blocked`）最大数量；运行中的工作永不修剪。设为 `0` 禁用按数量清理。 |
| `maxRunAgeDays` | `30` | 非活动运行的最大保留天数；运行中的工作永不修剪。设为 `0` 禁用按时间清理。 |

...
<Callout type="info">
  这是一份*模式*指南，不是宿主指南。它在 Pi（`/tf run`）和 Codex / Claude Code / OpenCode / Grok Build / Hermes Agent（`taskflow_run`）上的行为完全一致。宿主安装见 [Pi](/zh-cn/docs/guides/pi)、[Codex](/zh-cn/docs/guides/codex)、[Claude Code](/zh-cn/docs/guides/claude-code)、[OpenCode](/zh-cn/docs/guides/opencode)、[Grok Build](/zh-cn/docs/guides/grok-build) 或 [Hermes Agent](/zh-cn/docs/guides/hermes) 指南。
</Callout>
...
生成的子 flow 继承父运行的预算阈值，且不能声明更宽松的阈值。子的 `budget` 被钳制到它自己声明的阈值和父运行**剩余**阈值中较小的那个。这仍是基于已上报用量的止损线，因此活动调用可能超支。

这意味着模型没法给自己声明比父 flow 更宽松的止损线。父运行的 `budget: { maxUSD: 4.0 }` 是控制阈值，而不是保证的最大账单：普通调用串行准入，已经活动的 `race` 分支则都可能贡献超支。

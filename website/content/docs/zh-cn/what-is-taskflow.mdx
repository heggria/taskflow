...
taskflow 定义是 JSON（或 YAML、或 MDX frontmatter）。它声明**阶段**——离散任务节点（`agent`、`map`、`gate`、`reduce`、`loop`、`tournament`、`approval`、`flow`、`script`）——由 `dependsOn` 边连成一张 DAG，再加上 `when` 守卫、`join`、`retry`、`budget` 等控制流。

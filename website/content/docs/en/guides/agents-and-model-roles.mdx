...
| `maxKeptRuns` | `100` | Maximum inactive runs (`completed`, `failed`, `paused`, or `blocked`) to keep. Running work is never pruned. Set to `0` to disable count-based cleanup. |
| `maxRunAgeDays` | `30` | Maximum age (days) for inactive runs. Running work is never pruned. Set to `0` to disable age-based cleanup. |

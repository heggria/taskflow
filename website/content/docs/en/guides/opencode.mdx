...
|---|---|---|
| Read-only (no `write`/`edit`/`bash`) | `--pure` + injected default-deny policy | External plugins are disabled; only known read/list/search built-ins are allowed, so inherited custom and MCP tools are denied. |
...
Children inherit only platform/proxy/CA, OpenCode configuration, and supported provider variables. Unrelated parent secrets are removed. Add an intentional task variable by name with the comma-separated `PI_TASKFLOW_CHILD_ENV_ALLOW` operator setting.
...
`taskflow_run` is synchronous by default. For a long flow, set
`mode: "background"`; the MCP call returns a durable `runId` immediately.
Use `taskflow_runs` to list, inspect, wait for, or cancel that worker.
...
| `taskflow_reconcile_workspace` | Explicitly accept an inspected/repaired resolve-only workspace after a dirty-unknown write. |

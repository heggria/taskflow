...
| Phase kind | Behavior |
|------------|----------|
| Child isolation | ephemeral HERMES_HOME with non-secret model/fallback routing, a routed-provider-only inference `auth.json`, and provider-only dotenv; no parent MCP/skills/memory/rules; RO plugin only on RO children |

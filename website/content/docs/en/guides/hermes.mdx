...
| Phase kind | Behavior |
|------------|----------|
| Child isolation | ephemeral HERMES_HOME + `--ignore-rules` (show_reasoning off; RO file plugin) |
...
- `HERMES_HOME` for credentials (`.env`/`auth.json` load from ephemeral HERMES_HOME)

...
| Phase kind | Behavior |
|------------|----------|
| Child isolation | Always `--safe-mode` + ephemeral HERMES_HOME (credentials only) |
| Read-only tool whitelist | model-only by default (no tools/network); `PI_TASKFLOW_HERMES_READONLY_WEB=1` → `web,search` (never attach writable `file`) |

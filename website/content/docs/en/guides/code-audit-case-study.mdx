...
<Callout type="info">
  This is a *pattern* guide, not a host guide. It works the same on Pi (`/tf run`) and on Codex / Claude Code / OpenCode / Grok Build / Hermes Agent (`taskflow_run`). See the [Pi](/en/docs/guides/pi), [Codex](/en/docs/guides/codex), [Claude Code](/en/docs/guides/claude-code), [OpenCode](/en/docs/guides/opencode), [Grok Build](/en/docs/guides/grok-build), or [Hermes Agent](/en/docs/guides/hermes) guide for host-specific install.
</Callout>
...
<Callout type="info">
  taskflow parses gate verdicts leniently but fails closed for the truly unparseable. If a gate output contains no JSON, no `VERDICT:` marker, and no recognized verdict word, the runtime treats it as `BLOCK` (issue #54) — a gate that cannot reach a verdict cannot be trusted to pass. If you want a human to make the final call for a merge gate, add a downstream `approval` phase.
</Callout>
...
`budget: { maxUSD: 3.0 }` is an observed-usage stop-loss. Once reported accumulated cost exceeds it, the runtime admits no new call and skips remaining phases. An already-active call can overshoot, so this limits runaway spend rather than guaranteeing a $3 maximum. For a 47-file review on strong models, $3 may be comfortable; tune per repo, and remember the gate runs *before* the report, so a `BLOCK` verdict saves the report's tokens.
...
<Cards>
  <Card title="Phase Types" href="/en/docs/concepts/phases">
    The twelve building blocks — including `map`, `parallel`, `reduce`, and `gate` used here.
  </Card>
  ...
  <Card title="Budget" href="/en/docs/syntax/budget">
    Run-wide observed-usage stop-losses and how a `BLOCK` verdict saves downstream tokens.
  </Card>

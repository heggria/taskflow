...
<Callout type="info">
  These templates run identically on Pi (`/tf run`) and on Codex / Claude Code / OpenCode / Grok Build / Hermes Agent (`taskflow_run`). See the [Pi](/en/docs/guides/pi), [Codex](/en/docs/guides/codex), [Claude Code](/en/docs/guides/claude-code), [OpenCode](/en/docs/guides/opencode), [Grok Build](/en/docs/guides/grok-build), or [Hermes Agent](/en/docs/guides/hermes) guide for host-specific install.
</Callout>
...
Save any template as a `.json` file in your project, then **plan** (and optionally verify) it for free before running:

```bash title="Plan before you run (0.2.7)"
/tf plan <name> '{"dir":"src"}'   # bind args + budget bound, zero tokens
/tf verify <name>                 # structural checks
```

<Callout type="info">
  **0.2.7 starter templates** also live in the monorepo under [`examples/templates/`](https://github.com/heggria/taskflow/tree/main/examples/templates): `plan-first.json` (preflight habit) and `background-with-hooks.json` (file hooks + approval timeout). See the [0.2.7 release note](/en/docs/blog/plan-before-spend-0.2.7).
</Callout>
...
<Tabs groupId="host" items={['Pi', 'MCP hosts']}>
  ...
  <Tab value="MCP hosts">
    ```bash title="Run on Codex / Claude / OpenCode / Grok / Hermes"
    # Ask the host to call taskflow_run (MCP). Same tool on every MCP host:
...
- **`budget`** — a run-wide observed-usage stop-loss. A `BLOCK` verdict or observed breach prevents new downstream calls; calls already active may overshoot (including concurrent `race` branches).

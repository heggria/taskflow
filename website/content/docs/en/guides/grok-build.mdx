...
`taskflow_run` is synchronous by default. For a long graph, set `mode: "background"`; the call returns a durable `runId` immediately. Use `taskflow_runs` to list, inspect, wait for, or cancel it. The 30-minute tool timeout applies only to foreground calls.
...
| `taskflow_resume` | Fork a failed or paused run into a new immutable child run. |
| `taskflow_version` | Report package/build/schema/host identity. |
...
# or: grok mcp add taskflow -- npx -y -p grok-taskflow@0.2.3 grok-taskflow-mcp
```

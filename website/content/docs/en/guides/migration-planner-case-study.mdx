...
A generated sub-flow inherits the parent run's budget threshold and cannot declare a looser one. The child's `budget` is clamped to the smaller of its own declared threshold and the parent's **remaining** threshold. It remains an observed-usage stop-loss, so active calls may overshoot.

This means a model cannot grant itself a looser stop-loss than the parent flow declared. The parent's `budget: { maxUSD: 4.0 }` is the controlling threshold, not a guaranteed maximum: ordinary calls are admitted serially, while already-active `race` branches can all contribute overshoot.

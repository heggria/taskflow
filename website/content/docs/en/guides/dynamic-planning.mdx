...
A generated sub-flow inherits the parent run's budget threshold and cannot declare a looser one. The child's `budget` is clamped to the smaller of its own declared threshold and the parent's remaining threshold. This is still an observed-usage stop-loss: an active call can overshoot, and active `race` branches can overshoot together.

This means a model cannot declare a looser stop-loss threshold than the parent flow allowed; it does not turn observed usage into a guaranteed maximum.

...
|---|---|---|
| Read-only or omitted; no mutating/unknown requested tool | `--allowedTools Read,Grep,Glob,WebFetch,WebSearch` | Mutating tools are denied outright. Note there's no read-only shell — `Bash` is not granted. |
| Mutating or unknown requested tool | Rejected by default | Set `PI_TASKFLOW_CLAUDE_UNSAFE_BYPASS=1` only for trusted flows to opt into `bypassPermissions`. |

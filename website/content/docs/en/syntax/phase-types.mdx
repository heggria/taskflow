---
title: Phase Types
description: The twelve building blocks of a taskflow, and when to reach for each one.
---
...
You might be wondering: *do I need to learn all twelve before I can do anything useful?* No. Most flows are built from three or four types. The rest exist for specific situations — human approval, iterative refinement, competing drafts — and you can ignore them until you meet one of those situations.
...
| "Keep the first approach that succeeds" | `race` | First successful branch wins; hard-fail does not. |
...
<Callout type="tip">
  **Lenient parsing, fail-closed fallback.** taskflow tries hard to extract a verdict — JSON fields, `VERDICT:` tokens, and natural-language words. If the output cannot be parsed at all, the gate fails closed (`BLOCK`, issue #54). A confused gate cannot be trusted to pass. Config or resolution slips (unresolved `score.target`, malformed `scorers`) remain fail-open with a warning.
</Callout>
...
- **`timeoutMs` (0.2.7, optional)** → max wait for a human (`>= 1000` ms). Omit for infinite wait (previous default).
- **`onExpire` (0.2.7)** → `reject` (default) | `fail` | `approve` when the wait expires. `approve` is an explicit footgun.

<Callout type="warn">
  Approval only works in interactive hosts (Pi). In CI or detached runs, an `approval` phase **auto-rejects** — so don't put one on a flow you run unattended. Wall `timeout` (agent caps) and `cache.scope: "cross-run"` are not supported on approval phases; use **`timeoutMs`** for the HITL wait bound.
</Callout>
...
```json title="race — first success wins"
{
  "id": "quick",
  "type": "race",
  "branches": [
...
<Callout type="warn">
  **Security caps.** Like `flow{def}`, `expand` enforces max 100 phases, 200 map items, 16 concurrency, 5 nesting levels, no `script`, and `cwd` containment. In `graft` mode, child ids are promoted as `\u003cexpandId\u003e-\u003cchildId\u003e` and any `{steps.X}` references inside the fragment are rewritten with the same prefix.
</Callout>
...
| `task` | `string` | — | Task prompt. Supports interpolation. Required for `agent`/`map`/`reduce`/`loop`/`tournament` (unless `branches` or `taskFile` is used). |
| `taskFile` | `string` | — | Load-time include. A literal path relative to the flow definition directory. Trusted loaders (`defineFile` / saved flow) inline the UTF-8 body into `task` and delete `taskFile` before validate / interpolate / cache / FlowIR. Mutually exclusive with `task`. Path is **not** interpolated. Forbidden on inline `define` and on generated `flow{def}` / expand. Cap: 256 KiB. `taskflow-dsl` compile/`check` does **not** read the file. |
...
<Callout type="info">
  **Validation rules for common fields.**
  ...
  - `cache.scope: "cross-run"` is forbidden on `gate`, `approval`, `loop`, `tournament`, `script`, `race`, and `expand`.
...
- **`branches`** (`{task, agent?}[]`, required): at least 2 competing branches. The first successful branch wins; a fast hard-fail does not.
- **`cancelLosers`** (`boolean`, default `true`): after the first success, send `AbortSignal` to remaining branches (best-effort). Phase `usage` aggregates all branches for honest budgets. Set `false` to let losers finish.
- **`task`**: ignored — use `branches` instead.
...
- **`def`** (required): inline or runtime-generated fragment definition. A string is interpolated then JSON-parsed; an object is used directly.
- **`expandMode`** (`"nested"` | `"graft"`, default `"nested"`): `"nested"` isolates the fragment like `flow{def}`; `"graft"` promotes child phase states onto the parent as `\u003cexpandId\u003e-\u003cchildId\u003e`.
- **`maxNodes`** (`number`, default `50`, max `100`): caps the fragment size. Dynamic validation matches the `flow{def}` security caps.
- **`task`**: ignored.
...
| Error | Cause | Fix |
|---|---|---|
| `Phase 'x' (agent) requires 'task'` | `task` missing or empty. | Add a `task` string or a `taskFile`. |
| `'task' and 'taskFile' are mutually exclusive` | Both fields set. | Keep exactly one. |
| `TF_TASKFILE_NO_PROVENANCE` | `taskFile` on an inline `define`. | Use `defineFile` or a saved flow. |
| `TF_DYNAMIC_RESOURCE_FORBIDDEN` | `taskFile` on a generated sub-flow. | Inline the prompt as `task`. |

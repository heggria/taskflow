...
<Callout type="info">
  **`cancelLosers` is reserved.** The schema accepts it for forward compatibility, but losers are **not** aborted yet — they may run to natural completion. Cap spend with `budget` / `timeout`.
</Callout>

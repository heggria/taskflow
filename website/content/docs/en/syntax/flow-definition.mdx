...
## Notify when a run finishes: `hooks` (0.2.7)

Optional fire-and-forget notifications when the run reaches a terminal status. Payload is summary-only (`taskflow.hook.v1`) — never transcripts. Hook failure never changes run status.
...
- **webhook:** `https://…`, or `http://127.0.0.1` / `localhost` for local dogfood  
- **file:** project-relative path (no `..`)  
- **command:** argv array only (no shell string); receives `TASKFLOW_HOOK_*` env vars  

See the [0.2.7 release note](/en/docs/blog/plan-before-spend-0.2.7).

## Cap the cost: `budget`

Some flows are open-ended — a deep research loop, a tournament with many variants. A `budget` sets a run-wide observed-usage stop-loss for USD cost and/or tokens. Once either breach is reported, no new call is admitted, remaining phases are **skipped**, and the run ends with status `blocked`.
...
<Callout type="info">
  Both fields are optional — set either or both. Already-running subagents are not interrupted mid-call and may overshoot. Ordinary budgeted DAG/map/parallel/tournament calls are admitted serially; a `race` necessarily starts its competitors together. Codex accepts only `maxTokens` and rejects `maxUSD`; Grok rejects every budget. A budget of `0` is treated as "exceed immediately," useful only for dry structural checks.
</Callout>
...
- The cross-run-blocked phase types (`gate`, `approval`, `loop`, `tournament`, `script`, `race`, and `expand`) are **never** cached cross-run, even under `incremental` — they must produce a fresh result each run. In the example above, `lint` re-runs every invocation despite `incremental`; `review` is cached.
...
When several phases read the same files, you can opt them all into the Shared Context Tree with a single flag. `contextSharing: true` is shorthand for `shareContext: true` on every phase. In 0.2.0, the Pi adapter injects `ctx_read`/`ctx_write` and `ctx_report`/`ctx_spawn`; Codex, Claude, OpenCode, Grok, and Hermes runners do not inject these tools yet. Individual phases can still opt out with `shareContext: false`.
...
For one-off delegations you don't need to author a full `phases` DAG. taskflow accepts three shorthand shapes that the runtime's `desugar()` expands into a complete flow before validation. A spec is recognized as shorthand when it has **no** `phases` array **but** has one of `task`, `taskFile`, `tasks`, or `chain`.
...
Desugars to one `agent` phase (id `"main"`, `final: true`). Optional `agent`, `context`, and `contextLimit` carry through. A file-backed shorthand may use `taskFile` instead of `task` — the trusted loader inlines the file before desugar.
...
| `budget` | `Budget` | — | No | Run-wide observed-usage cost / token stop-loss. See [`budget`](#cap-the-cost-budget). |
...
| `scriptCwd` | `"invocation" \| "flow"` | `"invocation"` | No | Default cwd for script phases without an explicit phase `cwd`. `"flow"` requires a saved flow or `defineFile` source and uses that file's directory; inherited cwd-bridge boundaries still constrain the resolved directory. `taskFile` is the same class of trusted source: resolved against the definition file's directory at load time, never at runtime. |
...
| `Phase 'X': unknown type 'Y'` | `type` not in the 12 allowed values. |

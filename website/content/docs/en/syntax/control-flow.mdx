...
## `when`: skip a phase conditionally [#when]
...
## `join`: how a phase waits on its dependencies [#join]
...
## `optional`: tolerate failure [#optional]
...
## `retry`: try again on failure [#retry]
...
## `idempotent`: classify side effects [#idempotent]
...
## `onBlock`: self-healing gates [#onblock]
...
<Cards>
  ...
  <Card title="Phase Types" href="/en/docs/syntax/phase-types">
    The 12 phase types and every field on each.
  </Card>
</Cards>

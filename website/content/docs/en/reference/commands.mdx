...
<Callout type="tip">
  `peek`, `provenance`, `why-stale`, and `recompute` explain *what would re-run*. `trace` + `replay` explain *what was decided* and counterfactual knobs — see [Deterministic Replay](/en/docs/concepts/deterministic-replay).
</Callout>

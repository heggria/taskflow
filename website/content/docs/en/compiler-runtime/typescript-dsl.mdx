...
<Callout type="info">
  **Package status.** `taskflow-dsl` lives in the monorepo (`packages/taskflow-dsl`). It is **not** required for JSON authors. Package manifests are `0.2.2` on this release line; npm availability updates after the `v0.2.2` publish job — prefer a workspace install / local path until then.
</Callout>

...
<Callout type="info">
  **Package status.** `taskflow-dsl` lives in the monorepo (`packages/taskflow-dsl`). It is **not** required for JSON authors. Package manifests are `0.2.9` on this release line; install from npm after the `v0.2.9` publish job, or use a workspace / local path from this monorepo.
</Callout>

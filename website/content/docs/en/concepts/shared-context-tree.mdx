...
> **0.2.0 host scope:** `ctx_*` tools are injected by `pi-taskflow`. Codex,
> Claude, OpenCode, and Grok runners do not expose them yet, so `shareContext`
> is not portable to those hosts in this release.

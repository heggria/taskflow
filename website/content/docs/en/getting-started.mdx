...
<Callout type="info">
  This guide covers the stable 0.2.x host installation path. For the current **0.3 Trusted Effects candidate**, start with the [Trusted Effects overview](/en/docs/trusted-effects) and use the repository checkout; 0.3 is unreleased and not GA.
</Callout>
...
<Tabs groupId="host" items={['Pi', 'Codex', 'Claude', 'OpenCode', 'Grok', 'Hermes']}>
  <Tab value="Pi">
  ...
  <Tab value="Grok">
    ...
    # Requires the custom profile shown in Install below.
    # Ask Grok to call taskflow_verify / taskflow_run via MCP tools.
    ```
  </Tab>
  <Tab value="Hermes">
    ```bash title="Run on Hermes Agent"
    # Ask Hermes to call taskflow_verify / taskflow_run via MCP tools.
    ```
  </Tab>
</Tabs>
...
<Callout type="tip">
  **0.2.7 habit:** for any non-trivial flow, call **`taskflow_plan` / `/tf plan`** with the same `args` you will pass to `run`. It is free, binds typed args, and surfaces the worst-case agent-call bound before you spend. See [What's new in 0.2.7](/en/docs/blog/plan-before-spend-0.2.7).
</Callout>
...
<Callout type="warn">
  Every taskflow package requires **Node.js 22.19.0 or newer**.
</Callout>
...
<Tabs groupId="host" items={['Pi', 'Codex', 'Claude', 'OpenCode', 'Grok', 'Hermes']}>
  ...
  <Tab value="Grok">
    ...
    ```
    Read-only phases and mutating or omitted-tool phases each fail closed unless their matching custom profile is configured.
  </Tab>
  <Tab value="Hermes">
...
<Cards>
  ...
  <Card title="Core Concepts" href="/en/docs/concepts">
    Understand the DAG model and the twelve phase types.
  </Card>

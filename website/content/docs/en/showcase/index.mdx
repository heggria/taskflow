...
They are not host walkthroughs — the same flow runs identically on Pi (`/tf run`) and on Codex / Claude Code / OpenCode / Grok Build / Hermes (`taskflow_run`). They are *pattern* guides: read them to learn the shape, then adapt the shape to your own problem.
